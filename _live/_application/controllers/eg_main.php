...
class Eg_main extends Eg_BaseMobile_Controller {
		...
		if( is_file($file_path) ) {
			...
			foreach($morning_list as $morning) {
				if($morning['e_display_date'] <= date('Y-m-d H:i:s')) {
					...
					$cont_banner[$cont]['url'] = '/'.EG.'_stock/research_view/'.$morning['e_id'];
...
//echo '<pre>'; print_r($tab_stock_data);
    ...
    public function saveTicker() {
        ...
        $this->common->locationhref('/'.EG.'_main/search');
        exit;
    }

	public function service_info() {
        $this->load->view('/'.EG.'/main/service_info');
    }
}
