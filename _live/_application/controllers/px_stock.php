...
require_once dirname(__FILE__).'/px_base_mobile.php';
class Px_stock extends Px_BaseMobile_Controller {
    ...
    public function index() {
        //$this->common->locationhref('/PX_stock/choice_api');
        $this->common->locationhref('/'.PX.'_stock/recommend');
    }
    ...
    public function choice_api($type='') {
		...
		if($type=='A') {
			...
			*/
			...
			$result[0]['link'] = '/'.PX.'_stock/recommend';
			...
			$result[1]['link'] = '/'.PX.'_stock/recommend_view/'.$recommend[0]['rc_id'];
			...
			$result[2]['link'] = '/'.PX.'_attractiveness/attractive?sort=total&netincome=all&marketcap=over100billion';
    ...
    public function recommend($tab='1') {
		...
		*/
		...
		if($tab == '1') {
			$data['content_html'] = $this->load->view('/'.PX.'/stock/recommend_list.php', array('rc_list' => $rc_list, 'tab' => '1', 'is_event' => $this->is_event), true);
        ...
        $this->header_data['head_title'] = '팍스초이스';
        $this->header_data['head_url'] = '/';

        $data['meta_title'] = ($tab=='1') ? '종목추천 - 팍스초이스' : '포트폴리오 - 팍스초이스';
		//15min
    ...
    public function ajax_get_recommend_list() {
		...
		*/
        ...
        $content_html = $this->load->view('/'.PX.'/stock/recommend_list.php', array('rc_list' => $rc_list, 'tab' => '1'), true);
        echo $content_html;
        return;
    }
...
//print_r($data['mrt_list']);
        ...
        $this->header_data['back_url'] = '/'.PX.'_stock/recommend';
        //$this->header_data['back_url'] = '/';

		  
        $data['meta_title'] = (($rc_data['rc_title'] != '') ? $rc_data['rc_title'].' | ':'').'종목추천 - 팍스초이스';
		//15min
		$data['is_open'] = $this->is_open;
    ...
    public function recipe($type='dividend') {
        ...
        $data['score_content_html'] = $this->load->view('/'.PX.'/stock/recipe_list.php', array('type' => $type, 'list' => $list), true);
		$data['type'] = $type;
        ...
        $this->header_data['header_template'] = '6';
        $this->header_data['head_title'] = '투자 레시피';
        $this->header_data['back_url'] = '/'.PX.'_stock/recipe_intro';

		$data['meta_title'] = $data['title'].' | 투자레시피 - 팍스초이스';
    ...
    public function ajax_get_recipe_list() {
        ...
        $content_html = $this->load->view('/'.PX.'/stock/recipe_list.php', array('type' => $type, 'list' => $list), true);
        echo $content_html;
        return;
    }
    ...
    public function ajax_get_vod_list() {
        ...
        echo $this->load->view('/'.PX.'/stock/vod_mjm_list', $list_data, true);
    }
    ...
    public function vod_view($no=0) {
        ...
        $this->header_data['head_title'] = '동영상';
        $this->header_data['back_url'] = '/'.PX.'_stock/vod';

		$data['meta_title'] = $row['vm_title'].' | 동영상 - 팍스초이스';
    ...
    public function ajax_get_research_list($more) {
        ...
        echo $this->load->view('/'.PX.'/stock/research_list', $list_data, true);
    }
    ...
    public function research_view($no=0) {
		...
		$row['e_content'] = preg_replace("/<a[^>]*>/i", '', $row['e_content']);
		$row['e_content'] = preg_replace("/<\/a>/i", '', $row['e_content']);
		...
		if($row['e_class']=='1') {
			...
			$this->header_data['head_title'] = '미국주식 탐구생활';
			$this->header_data['back_url'] = '/'.PX.'_stock/research';
			$data['meta_title'] = $row['e_title'].' | 탐구생활 - 팍스초이스';
			$data['back_url'] = '/'.PX.'_stock/research';
		}
		else {
			$this->header_data['head_title'] = '미국주식 종목노트';
			$this->header_data['back_url'] = '/'.PX.'_stock/note';
			$data['meta_title'] = $row['e_title'].' | 종목노트 - 팍스초이스';
			$data['back_url'] = '/'.PX.'_stock/note';
		}
    ...
    public function ajax_get_morning_list() {
		...
		$list_data['is_event'] = $this->is_event;
        echo $this->load->view('/'.PX.'/stock/morning_list', $list_data, true);
    }
    ...
    public function morning_view($no=0) {
		...
		*/
        ...
        //$this->header_data['header_template'] = '4';
        $this->header_data['head_title'] = '모닝브리핑';
        $this->header_data['back_url'] = '/'.PX.'_stock/morning';
        //$this->header_data['back_url'] = '/';
		$data['meta_title'] = $row['mo_title'].' | 모닝브리핑 - 팍스초이스';
    ...
    public function alarm() {
        ...
        $params['>=']['nt_display_date'] = $this->start_day;
	...
	public function push_control($push='') {
        ...
        if( in_array($push, array('Y', 'N'))) {
			...
			$this->common->locationhref('/'.PX.'_stock/view_close');
			exit;
		}
		else {
				...
				foreach($ticker_list as $key => $ticker) {
					...
					if($ticker['tkr_ticker']!='') {
						...
						if(sizeof($recom_data) > 0) {
							...
							$ticker_info[$ticker['tkr_ticker']]['recom_link'] = '/'.PX.'_stock/recommend_view/'.$recom['rc_id'];
						}
						...
						if($ticker_info[$ticker['tkr_ticker']]['mri']['m_biz_dividend_score'] >= 16) {
							$recipe = '<li><a href="/'.PX.'_stock/recipe/dividend">배당주</a></li>'."\n";
						}
						if($ticker_info[$ticker['tkr_ticker']]['mri']['m_biz_growth_score'] >= 16) {
							$recipe .= '<li><a href="/'.PX.'_stock/recipe/growth">성장주</a></li>'."\n";
						}
						if($ticker_info[$ticker['tkr_ticker']]['mri']['m_biz_moat_score'] >= 16) {
							$recipe .= '<li><a href="/'.PX.'_stock/recipe/moat">가치주</a></li>'."\n";
						}
						...
						$ru = urlencode(PX.'_stock/catch_info');
						...
						if($choice_ticker != '' && $choice_name != '') {
							$industry_commnet = '<span class="same_selt"><a href="/'.PX.'_search/invest_charm/'.$choice_ticker.'">'.$choice_name.'</a>와(과) 같은 업종입니다</span>';
						}
$pre = PX;
$catch_list = <<<PHPSKIN
...
PHPSKIN;
				...
				if($catch_all!='') {
					...
					echo $catch_all;
					if($is_next === true) {
						echo '<div class="next"><a href="/'.PX.'_stock/add_catch?mode=auto&idx='.$idx.'&page='.$nextPage.'" class="nextPage">다음 페이지</a></div>';
					}
				}
			}
		}
				...
				foreach($ticker_list as $key => $ticker) {
					...
					if($ticker['tkr_ticker']!='') {
						...
						if(sizeof($analysis_data) > 0) {
							...
							$ticker_info[$ticker['tkr_ticker']]['analysis_link'] = '/'.PX.'_stock/analysis_view/'.$analy['an_id'];
						}
						*/
    ...
    public function set_catch() {
		...
		//$this->common->locationhref('/'.PX.'_stock/catch_info?catch_list='.$c_tickerlist.'&sector_list='.$c_sector);
		//exit;
	}
