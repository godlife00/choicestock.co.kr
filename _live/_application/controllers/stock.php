...
class Stock extends BaseMobile_Controller {
    ...
    public function getOgTags($html) {
        //원본 : $pattern='/<\s*meta\s*property="og:([^"]+)"\s+content="([^"]*)/i';
        $pattern='/<meta[^>]*property="og:([^"]+)"\s+content="([^"]*)/i';
    ...
    public function news_test() {
        ...
        $meta_tags['title'] = iconv("euc-kr", "utf-8", $meta_tags['title']); 
        $meta_tags['description'] = iconv("euc-kr", "utf-8", $meta_tags['description']); 
        ...
        if(strtoupper($matches[3]) == "EUC-KR" ){
            echo '<br>=======================<br>';
            $txt = iconv("euc-kr", "utf-8", $txt);
        }

        $return = $this->getOgTags($txt);
        ...
        //뉴스수신 테스트
        $mode         = 'I';
        $id           = '202103040001';//뉴스ID
        $title        = '글로벌이코노믹 기사 테스트1';        //뉴스제목
    
        $content      = '글로벌이코노믹 기사 테스트(본문)';  //뉴스본문

        $display_date = date('Y-m-d H:i:s');    //뉴스표출시간
        $writer       = '글로벌이코노믹 기자';  //작성자+직책
        $ticker       = '';                        //종목코드
        $pt_code      = 'CSPART003';            //업체코드(글로벌이코노믹:CSPART003)
...
//echo '=============<pre>'; print_r($data); exit;
        ...
        $postURL = 'https://www.choicestock.co.kr/stock/news_api'; 
        $response = $this->reqBasePost($data, $postURL);     //승인 호출
        $response = json_decode( $response, true );
        echo '<pre><br>news result--><br>';
        print_r($response);
        echo '<--news result';
    }
    ...
    public function news_api() {
        ...
        $mode = $mode;         //mode(I, U, D)

        if(!in_array($mode, array('I', 'U', 'D'))) {
            $result['result'] = 'FAIL';
            $result['retmsg'] = 'parameter error[mode::'.$mode.']';
            echo json_encode($result);
            exit;
        }
        ...
        $id           = $id;           //뉴스ID
        $title        = $title;        //뉴스제목
        $content      = $content;      //뉴스본문
        $content      = str_replace('</figcaption></figure>', '</figcaption></figure><br />', $content);
        $display_date = $display_date; //뉴스표출시간
        $writer       = $writer;       //작성자+직책
        $ticker       = $ticker;       //종목코드
        $pt_code      = $pt_code;      //업체코드(글로벌이코노믹:CSPART003)

		if($mode == 'D') {
			if($id == '' || $title == '' || $content == '' || $writer == '' || $pt_code == '') {
				$result['result'] = 'FAIL';
				$result['retmsg'] = 'delete parameter null[id::'.$id.']';
				echo json_encode($result);
				exit;
			}
		}
		else {
			if($id == '' || $title == '' || $content == '' || $display_date == '' || $writer == '' || $pt_code == '') {
				$result['result'] = 'FAIL';
				$result['retmsg'] = 'parameter null[id::'.$id.']';
				echo json_encode($result);
				exit;
			}
		}

        $logfile = fopen(LOG_PATH."/news_api_result.log", "a+" );

        fwrite( $logfile,"************************************************\r\n");
        fwrite( $logfile,"mode         : ".$mode."\r\n");
        fwrite( $logfile,"title        : ".$title."\r\n");
        fwrite( $logfile,"content      : ".$content."\r\n");
        fwrite( $logfile,"display_date : ".$display_date."\r\n");
        fwrite( $logfile,"writer       : ".$writer."\r\n");
        fwrite( $logfile,"ticker       : ".$ticker."\r\n");
        fwrite( $logfile,"pt_code      : ".$pt_code."\r\n");
        ...
        if(isset($matches[1][0]) && strlen($matches[1][0]) > 0) {
            $n_thumbnail = $matches[1][0];
        }

        if($mode == 'I' || $mode == 'U') {
            if($this->news_tb_model->get(array('n_code' => $n_code, 'n_pt_code' => $pt_code, 'n_pt_newsid' => $id))->isSuccess()) {
                $dbrow = $this->news_tb_model->getData();

                $update_params = array(
                    //'n_code' => $n_code,
                    //'n_writer_id' => $n_writer_id,
                    'n_writer_name' => $writer,
                    'n_title' => $title,
                    'n_tickers' => $ticker,
                    'n_content' => $content,
                    //'n_display_date' => '0000-00-00 00:00:00',
                    //'n_view_count' => ,
                    //'n_is_active' => 'NO',
                    //'n_view_srv' => ,
                    ...
                    //'n_pt_code' => $pt_code,
                    //'n_pt_newsid' => $id,
                    'n_pt_date' => $display_date,
                    //'n_created_at' => date('Y-m-d H:i:s'),
                    'n_updated_at' => date('Y-m-d H:i:s'),
                );
                
                $this->news_tb_model->doUpdate($dbrow['n_id'], $update_params);
            }
            else {
                ...
                $this->news_tb_model->doInsert($insert_params);
            }        
        }
        else {
            if($this->news_tb_model->get(array('n_code' => 'CS01', 'n_pt_code' => $pt_code, 'n_pt_newsid' => $id))->isSuccess()) {
                $dbrow = $this->news_tb_model->getData();
                $this->news_tb_model->doDelete($dbrow['n_id']);
            }
        }

        $result['result'] = 'OK';
        $result['retmsg'] = 'news transfer completed [id::'.$id.']';
        echo json_encode($result);
        exit;
    }
    ...
    public function choice_api($type='') {
        ...
        if($type=='A') {
            ...
            */
            ...
            $result[0]['link'] = '/'.X1.'_stock/recommend';
            ...
            $result[1]['link'] = '/'.X1.'_stock/recommend_view/'.$recommend[0]['rc_id'];
            ...
            $result[2]['link'] = '/'.X1.'_attractiveness/attractive?sort=total&netincome=all&marketcap=over100billion';
            ...
            $popular_ticker = array('TSLA', 'AAPL', 'AMZN', 'GOOGL', 'MSFT', 'NVDA', 'HAS', 'BA', 'DIS', 'PLTR', 'T', 'U', 'ASML', 'FB', 'XOM', 'BMO', 'MA', 'BABA', 'QCOM', 'AMD', 'SBUX', 'BRK.B', 'KO', 'CCL', 'O', 'NKE', 'DAL', 'NIO', 'PYPL', 'BIDU', 'UBER', 'NNOX', 'DZSI', 'NKLA', 'PFE', 'SQ');
            
            $params = array();
            $params['in']['tkr_ticker'] = $popular_ticker;
            $extra = array(
                'fields' => array('tkr_ticker', 'tkr_name', 'tkr_name_en', 'tkr_close', 'tkr_rate'),
                'order_by' => ''
            );
            $extra['cache_sec'] = 3600;
            $extra['slavedb'] = true;
            $popular_stock_data = $this->ticker_tb_model->getList($params, $extra)->getData();
            $popular_stock_data = $this->common->indexSort($popular_ticker, $popular_stock_data, 'tkr_ticker');
    ...
    public function recommend($tab='1') {
		...
		*/
        ...
        if($tab == '1') {
            $data['content_html'] = $this->load->view('/mobile/stock/recommend_list.php', array('rc_list' => $rc_list, 'tab' => '1', 'is_event' => $this->is_event), true);
    ...
    public function ajax_get_recommend_list() {
        ...
        */
        ...
        $content_html = $this->load->view('/mobile/stock/recommend_list.php', array('rc_list' => $rc_list, 'tab' => '1', 'is_event' => $this->is_event), true);
        echo $content_html;
        return;
    }
...
//print_r($data['mrt_list']);
    ...
    public function recipe($type='dividend') {
        //$this->load->model(DBNAME.'/mri_tb_model');
//관심주 top50 select count(*) cnt, mi_ticker from myitem_tb group by mi_ticker order by cnt desc
        if($type == 'performance') {
            $this->common->locationhref('/stock/recipe/earnings');
            exit;
        }
        ...
        $data['score_content_html'] = $this->load->view('/mobile/stock/recipe_list.php', array('type' => $type, 'list' => $list, 'is_open' => $this->is_open), true);
        $data['type'] = $type;

        $this->header_data['header_template'] = '6';
        $this->header_data['head_title'] = '투자 레시피';
        $this->header_data['back_url'] = '/stock/recipe_intro';
    ...
    public function ajax_get_recipe_list() {
        ...
        $content_html = $this->load->view('/mobile/stock/recipe_list.php', array('type' => $type, 'list' => $list, 'is_open' => $this->is_open), true);
        echo $content_html;
        return;
    }
    ...
    public function ajax_get_vod_list() {
        ...
        echo $this->load->view('/mobile/stock/vod_mjm_list', $list_data, true);
    }
    ...
    public function vod_view($no=0) {
        ...
        //if($row['vm_is_active'] != 'Y' || $row['vm_display_date'] > date('Y-m-d H:i:s') || ($row['vm_view_srv'] != '' && $row['vm_view_srv'] != 'C')) {
        if($row['vm_view_cs'] != 'Y' || $row['vm_display_date'] > date('Y-m-d H:i:s')) {
            $this->common->historyback();
            return;
        }
        ...
        $this->header_data['head_title'] = '동영상';
        $this->header_data['back_url'] = '/stock/vod';

        $data['meta_title'] = $row['vm_title'].' | 동영상 - 초이스스탁US';
    ...
    public function research_view($no=0) {
        ...
        //if($row['e_is_active'] != 'Y' || $row['e_display_date'] > date('Y-m-d H:i:s') || ($row['e_view_srv'] != '' && $row['e_view_srv'] != 'C')) {
        if($row['e_view_cs'] != 'Y' || $row['e_display_date'] > date('Y-m-d H:i:s')) {
            $this->common->historyback();
            return;
        }
        ...
        if($row['e_class']=='3') {
            $row['e_content'] =  $this->auto_link($row['e_content']);
        }
        ...
        if($row['e_class']=='1') {
            ...
            if($row['e_thumbnail'] != '') $data['meta_img'] = 'https://hero.datahero.co.kr'.$row['e_thumbnail'];
            $data['back_url'] = '/stock/research';

        }
        else {
            $this->header_data['head_title'] = '미국주식 종목노트';
            $this->header_data['back_url'] = '/stock/note';
            $data['meta_title'] = $row['e_title'].' | 종목노트 - 초이스스탁US';
            $data['back_url'] = '/stock/note';
        }
    ...
    public function morning() {
        ...
        $this->header_data['header_template'] = '20';
    ...
    public function ajax_get_morning_list() {
        ...
        $list_data['is_event'] = $this->is_event;
        echo $this->load->view('/mobile/stock/morning_list', $list_data, true);
    }
    ...
    public function morning_view($no=0) {
        ...
        //if($row['e_display_date'] > date('Y-m-d H:i:s') || $row['e_is_active'] == 'NO' || $row['e_class'] != '3') {
        if($row['e_display_date'] > date('Y-m-d H:i:s') || $row['e_view_cs'] != 'Y' || $row['e_class'] != '3') {
            $this->common->historyback();
            return;
        }
        ...
        $this->header_data['head_title'] = '모닝브리핑';
        $this->header_data['back_url'] = '/stock/morning';
        //$this->header_data['back_url'] = '/';
    ...
    public function push_control($push='') {
        ...
        if( in_array($push, array('Y', 'N'))) {
            ...
            $this->common->locationhref('/stock/view_close');
            exit;
        }
        else {
        ...
        if( is_file($file_path) ) {
            ...
            foreach($ticker_list as $ticker) {
                ...
                if($ticker_info[$ticker['tkr_ticker']]['mri']['m_biz_dividend_score'] >= 16) {
                    $recipe = '<li><a href="/stock/recipe/dividend">배당주</a></li>'."\n";
                }
                if($ticker_info[$ticker['tkr_ticker']]['mri']['m_future_score'] >= 16) {
                    $recipe .= '<li><a href="/stock/recipe/growth">성장주</a></li>'."\n";
                }
                if($ticker_info[$ticker['tkr_ticker']]['mri']['m_biz_moat_score'] >= 16) {
                    $recipe .= '<li><a href="/stock/recipe/moat">가치주</a></li>'."\n";
                }
                ...
                if($choice_ticker != '' && $choice_name != '') {
                    $industry_commnet = '<span class="same_selt"><a href="/search/summary/'.$choice_ticker.'">'.$choice_name.'</a>와(과) 같은 업종입니다</span>';
                }
$catch_list = <<<PHPSKIN
<div class="chart_catch">
    <div class="data_area">
        <h2 class="title"><a href="/search/summary/{$ticker['tkr_ticker']}#">{$ticker_info[$ticker['tkr_ticker']]['tkr_name']}</a></h2>
        <ul class="info">
...
PHPSKIN;
            ...
            if($catch_all!='') {
                ...
                echo $catch_all;
                if($is_next === true) {
                    echo '<div class="next"><a href="/stock/add_catch2?mode=auto&idx='.$idx.'&page='.$nextPage.'" class="nextPage">다음 페이지</a></div>';
                }
            }
        }
    }
...
PHPSKIN;
                ...
                if($catch_all!='') {
                    ...
                    echo $catch_all;
                    if($is_next === true) {
                        echo '<div class="next"><a href="/stock/add_catch?mode=auto&idx='.$idx.'&page='.$nextPage.'" class="nextPage">다음 페이지</a></div>';
                    }
                }
            }
        }
    ...
    public function catch_info() {
        ...
        $is_file = false;
/*
        $catch_file ='kakao_1342320378.json';
        $file_path = WEBDATA.'/catch/'.$catch_file;
                ...
                if(is_array($catch_list) && sizeof($catch_list)>0 && $catch_list['c_tickerlist'] != '') {
                    ...
                    foreach($ticker_list as $key => $ticker) {
                        ...
                        if($ticker['tkr_ticker']!='') {
                            ...
                            $params['=']['m_ticker'] = $ticker['tkr_ticker'];
                            $extra = array(
                                'fields' => array('m_future_score', 'm_future_score', 'm_biz_moat_score', 'm_biz_total_score', 'm_v_fairvalue1', 'm_v_fairvalue2', 'm_v_fairvalue3', 'm_v_fairvalue4', 'm_v_fairvalue5', 'm_close', 'm_scalemarketcap'),
                                'slavedb' => true,
                            );
...
//echo '<pre>'; print_r($data); exit;
    ...
    public function catch_process() {
        ...
        $this->common->locationhref('/stock/catch_info?catch_list='.$c_tickerlist.'&sector_list='.$c_sector);
        exit;
    }
...
//echo $user_id; exit;
    ...
    public function makeDailyCatch() {
        echo "\n".'['.date("Y-m-d H:i:s")."] makeDailyCatch start!!\n";
        ...
        foreach($catch_userlist as $catch) {
            $this->_makeCatchFile($catch['c_user_id'], true);
        }

        echo "\n".'['.date("Y-m-d H:i:s")."] makeDailyCatch success!!\n";
    }
/*
    private function _get_fairvalue($value) {
        $result = array();
        $result['fairvalue5'] = @sprintf('%0.2f', $value*0.591);    //2021.01.15 수정 0.7 ->  0.591
        $result['fairvalue4'] = @sprintf('%0.2f', $value*0.769);    //2021.01.15 수정 0.85 -> 0.769
        $result['fairvalue3'] = $value;
        $result['fairvalue2'] = @sprintf('%0.2f', $value*1.3);    //2021.01.15 수정 1.176 -> 1.3
        $result['fairvalue1'] = @sprintf('%0.2f', $value*1.69);        //2021.01.15 수정 1.43 ->  1.69
        return $result;
    }
    ...
    public function winner_data() {
        ...
        if( is_file($file_path) ) {
             ...
             $winner_list = array_shift($winner_list);
        }

        //echo '<pre>'; print_r($winner_list);

        foreach($winner_list as $win) {
            if($win['win_close'] >= 200) continue;
            //티커/ 종가/ 주가추이(단/중/장기)/ 종목진단점수/ 매매신호(적극매수~ 등)
            ...
            $fairValue = $this->_get_fairvalue($win['win_fairvalue']);

            if($win['win_close'] < $fairValue['fairvalue4']) {
                $fair_tit = '적극매수'; 
            }
            else if($fairValue['fairvalue4'] <= $win['win_close'] && $win['win_close'] < $fairValue['fairvalue3']) {
                $fair_tit = '매수'; 
            }
            else if($fairValue['fairvalue3'] <= $win['win_close'] && $win['win_close'] < $fairValue['fairvalue2']) {
                $fair_tit = '보유'; 
            }
            else if($fairValue['fairvalue2'] <= $win['win_close'] && $win['win_close'] < $fairValue['fairvalue1']) {
                $fair_tit = '비중축소'; 
            }
            else {
                $fair_tit = '매도'; 
            ...
            echo $win['win_ticker'].'@'.$win['win_close'].'@'.$win['win_short'].'@'.$win['win_medium'].'@'.$win['win_long'].'@'.$win['win_biz_score'].'@'.$fair_tit.'<br>';
        }
    }
*/
    ...
    public function makeInterest() {
        ...
        $params['=']['mi_like'] = 'Y';
        $extra = array();
        $extra['group_by'] = 'mi_ticker';
        $extra['order_by'] = 'cnt desc';
        $extra['fields'] = array(
            'mi_ticker', 
            'count(mi_ticker) as cnt', 
        );
        $extra['limit'] = 50;
        $extra['slavedb'] = true;

        $interest_ticker = array_keys($this->common->getDataByPK($this->myitem_tb_model->getList($params, $extra)->getData(), 'mi_ticker'));
        $interest_list = $this->common->getDataByPK($this->myitem_tb_model->getList($params, $extra)->getData(), 'mi_ticker');
        //echo '<pre>'; print_r($interest_list);
        ...
        if(is_array($interest_ticker) && sizeof($interest_ticker) > 0) {
            $params = array();
            $params['in']['m_ticker'] = $interest_ticker;
            $extra = array(
                'fields' => array('m_ticker', 'm_future_score', 'm_biz_moat_score', 'm_biz_dividend_score', 'm_biz_total_score', 'm_v_fairvalue3'),
                'order_by' => '',
                'slavedb' => true,
            );

            $interest_stock_data = $this->mri_tb_model->getList($params, $extra)->getData();
        //echo '<pre>'; print_r($interest_stock_data); exit;

            foreach($interest_stock_data as $key => $val) {
                $interest_stock_data[$key]['ticker'] = $this->ticker_info_map[$val['m_ticker']];
                $interest_stock_data[$key]['expected_star'] = $this->cal_valuation($val['m_v_fairvalue3'], $interest_stock_data[$key]['ticker']['tkr_close']);
                $interest_stock_data[$key]['winner'] = $winner_all[$val['m_ticker']];
                $interest_stock_data[$key]['cnt'] = $interest_list[$val['m_ticker']]['cnt'];
            }
            $interest_stock_data = $this->common->indexSort($interest_ticker, $interest_stock_data, 'm_ticker');
        }    
        

        if(is_array($interest_stock_data)) {
            $interest_stock_data = json_encode($interest_stock_data);
            $interest_file = 'interest.json';
            $file_path = WEBDATA.'/json/'.$interest_file;
            $file_backpath = $file_path . '.bak';

            touch($file_backpath);
            file_put_contents($file_backpath, $interest_stock_data);
            rename($file_backpath, $file_path);
        }

        unset($interest_stock_data);
        unset($winner_all);
    }

    public function makeDividend() {
        //배당주
        $this->load->model(DBNAME.'/mri_tb_model');
        ...
        $dividend = $this->mri_tb_model->getRecomStockList('dividend', 4, $params, $extra);

        if(is_array($dividend)) {
            $dividend = json_encode($dividend);
            $dividend_file = 'dividend.json';
            $file_path = WEBDATA.'/json/'.$dividend_file;
            $file_backpath = $file_path . '.bak';

            touch($file_backpath);
            file_put_contents($file_backpath, $dividend);
            rename($file_backpath, $file_path);
        }
    ...
    public function makeSoaring() {
        ...
        $soaring_ticker = array_keys($this->common->getDataByPK($this->ticker_tb_model->getList($params, $extra)->getData(), 'tkr_ticker'));
/*
        $ticker_tb_data = array_keys($this->common->getDataByPK($this->ticker_tb_model->getList($params, $extra)->getData(), 'tkr_ticker'));
        
        //snp500체크
        $snp500 = 'snp500.json';
        //$file_path = WEBDATA.'/json/'.$snp500;
        $file_path = str_replace('hoon','datahero',WEBDATA).'/json/'.$snp500;

        if( is_file($file_path) ) {
             $snp500_list = json_decode(file_get_contents($file_path), true);
        }
        ...
        foreach($ticker_tb_data as $ticker) {
            if($cnt>49) break;
            if(in_array($ticker, $snp500_list)) {
                $soaring_ticker[] = $ticker;
                $cnt++;
            }
        }

        unset($ticker_tb_data);
*/
        //단기,중기,장기 체크
        $winner_all_file = 'winner_all.json';
        $file_path = str_replace('hoon','datahero',WEBDATA).'/'.$winner_all_file;
        ...
        if(is_array($soaring_ticker) && sizeof($soaring_ticker) > 0) {
            ...
            foreach($soaring_stock_data as $key => $val) {
                ...
                $soaring_stock_data[$key]['winner'] = $winner_all[$val['m_ticker']];
            }
            $soaring_stock_data = $this->common->indexSort($soaring_ticker, $soaring_stock_data, 'm_ticker');
        }    
        
        $soaring_all['one'] = $soaring_stock_data;

        unset($soaring_stock_data);

        $params = array();
        $params['raw'] = array('tkr_lastpricedate = (select max(tkr_lastpricedate) from ticker_tb)');

        $extra = array(
            'fields' => array('tkr_ticker','tkr_close'),
            'order_by' => 'tkr_ticker desc',
            'slavedb' => true
        );

        $ticker_list = $this->common->getDataByPK($this->ticker_tb_model->getList($params, $extra)->getData(), 'tkr_ticker');
        $ticker_list_cp = $ticker_list;

        $params = array();
        $params['>']['sep_date'] = date('Y-m-d', time()-86400*100);
        $extra = array(
            'fields' =>  array('distinct(sep_date)'),
            'order_by' => 'sep_date desc',
            'quandldb' => true,
        );
 
        $sep_date = $this->sep_tb_model->getList($params, $extra)->getData();

        $cnt=1;
        foreach($sep_date as $sep) {
            if($cnt==20) $twenty = $sep['sep_date'];
            if($cnt==60) $sixty = $sep['sep_date'];
            $cnt++;
        }
        ...
        //20일 급등주
        $params = array();
        $params['=']['sep_date'] = $twenty;
        $extra = array(
            //'fields' =>  array('sep_ticker', 'sep_dividends'),
            'fields' =>  array('sep_ticker', 'sep_close'),
            'order_by' => 'sep_ticker desc',
            'quandldb' => true,
        );

        $sep_list = $this->common->getDataByPK($this->sep_tb_model->getList($params, $extra)->getData(), 'sep_ticker');

        foreach($ticker_list as $key=>$val) {
            if($sep_list[$key]['sep_close'] > 0) {
                $ticker_list[$key]['sep_close'] = $sep_list[$key]['sep_close'];
                $ticker_list[$key]['diff'] = sprintf('%.2f', (($ticker_list[$key]['tkr_close'] - $sep_list[$key]['sep_close'])/$sep_list[$key]['sep_close'])*100);
            }
        }
        ...
        array_multisort($sort, SORT_DESC, $ticker_list);
        ...
        foreach($ticker_list as $ticker) {
            if($cnt>49) break;
            //if(in_array($ticker['tkr_ticker'], $snp500_list)) {
                $soaring_ticker[] = $ticker['tkr_ticker'];
                $cnt++;
            //}
        }
        ...
        if(is_array($soaring_ticker) && sizeof($soaring_ticker) > 0) {
            ...
            foreach($soaring_stock_data as $key => $val) {
                $soaring_stock_data[$key]['ticker'] = $this->ticker_info_map[$val['m_ticker']];
                $soaring_stock_data[$key]['expected_star'] = $this->cal_valuation($val['m_v_fairvalue3'], $soaring_stock_data[$key]['ticker']['tkr_close']);
                $soaring_stock_data[$key]['diff'] = $ticker_list[$val['m_ticker']]['diff'];
                $soaring_stock_data[$key]['winner'] = $winner_all[$val['m_ticker']];
            }
            $soaring_stock_data = $this->common->indexSort($soaring_ticker, $soaring_stock_data, 'm_ticker');
        }    

        $soaring_all['twenty'] = $soaring_stock_data;
        ...
        foreach($ticker_list_cp as $key=>$val) {
            if($sep_list[$key]['sep_close'] > 0) {
                $ticker_list_cp[$key]['sep_close'] = $sep_list[$key]['sep_close'];
                $ticker_list_cp[$key]['diff'] = sprintf('%.2f', (($ticker_list_cp[$key]['tkr_close'] - $sep_list[$key]['sep_close'])/$sep_list[$key]['sep_close'])*100);
            }
        }
        ...
        array_multisort($sort, SORT_DESC, $ticker_list_cp);

        unset($sep_list);

        $cnt=0;
          $soaring_ticker = array();
        foreach($ticker_list_cp as $ticker) {
            if($cnt>49) break;
            //if(in_array($ticker['tkr_ticker'], $snp500_list)) {
                $soaring_ticker[] = $ticker['tkr_ticker'];
                $cnt++;
        ...
        if(is_array($soaring_ticker) && sizeof($soaring_ticker) > 0) {
            ...
            foreach($soaring_stock_data as $key => $val) {
                $soaring_stock_data[$key]['ticker'] = $this->ticker_info_map[$val['m_ticker']];
                $soaring_stock_data[$key]['expected_star'] = $this->cal_valuation($val['m_v_fairvalue3'], $soaring_stock_data[$key]['ticker']['tkr_close']);
                $soaring_stock_data[$key]['diff'] = $ticker_list_cp[$val['m_ticker']]['diff'];
                $soaring_stock_data[$key]['winner'] = $winner_all[$val['m_ticker']];
            }
            $soaring_stock_data = $this->common->indexSort($soaring_ticker, $soaring_stock_data, 'm_ticker');
        }    

        $soaring_all['sixty'] = $soaring_stock_data;

        unset($soaring_stock_data);
        unset($ticker_list_cp);
        unset($winner_all);
        
        if(is_array($soaring_all)) {
            $soaring_main = array_slice($soaring_all['twenty'],0,4);

            $soaring_main = json_encode($soaring_main);
            $soaring_file = 'soaring_main.json';
            $file_path = WEBDATA.'/json/'.$soaring_file;
            $file_backpath = $file_path . '.bak';

            touch($file_backpath);
            file_put_contents($file_backpath, $soaring_main);
            rename($file_backpath, $file_path);

            $soaring_one = $soaring_all['one'];
            $soaring_one = json_encode($soaring_one);
            $soaring_file = 'soaring_one.json';
            $file_path = WEBDATA.'/json/'.$soaring_file;
            $file_backpath = $file_path . '.bak';

            touch($file_backpath);
            file_put_contents($file_backpath, $soaring_one);
            rename($file_backpath, $file_path);
            
            $soaring_all = json_encode($soaring_all);
            $soaring_file = 'soaring_all.json';
            $file_path = WEBDATA.'/json/'.$soaring_file;
            $file_backpath = $file_path . '.bak';

            touch($file_backpath);
            file_put_contents($file_backpath, $soaring_all);
            rename($file_backpath, $file_path);
        }
