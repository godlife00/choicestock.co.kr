...
class X1_main extends X1_BaseMobile_Controller {
	...
	public function index()
	{
		...
		if( is_file($file_path) ) {
            $file_data = array_shift(json_decode(file_get_contents($file_path), true));
			$data['bn1_txt'] = iconv_substr($file_data['e_title'], 0, 40, 'utf-8');
			$data['bn1_id'] = $file_data['e_id'];
		}
		else {
			$data['bn1_txt'] = '';
			$data['bn1_id'] = '';
    ...
    public function ajax_save_search_history($srv='X') {
...
//echo '<pre>'; print_r($tab_stock_data);
    ...
    public function service() {
        ...
        $this->header_data['back_url'] = '/'.X1.'_main';
		$data = array();
		$data['is_event'] = $this->is_event;
	...
	public function saveTicker() {
		...
		$this->common->locationhref('/'.X1.'_main/search');
		exit;
	}
}
