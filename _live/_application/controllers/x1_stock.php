...
class X1_stock extends X1_BaseMobile_Controller {
    ...
    public function index() {
        //$this->common->locationhref('/x1_stock/choice_api');
        $this->common->locationhref('/'.X1.'_stock/recommend');
    }
...
*/
		...
		*/
		...
		if($tab == '1') {
			$data['content_html'] = $this->load->view('/'.X1.'/stock/recommend_list.php', array('rc_list' => $rc_list, 'tab' => '1', 'is_event' => $this->is_event), true);
			...
			//무료회원오픈처리(21.11.04)
			set_cookie('free_ticker', $portfolio_list[0]['rc_id'].'|'.$portfolio_list[1]['rc_id'].'|'.$portfolio_list[2]['rc_id'], time()+86400);
    ...
    public function ajax_get_recommend_list() {
		...
		*/
        ...
        $content_html = $this->load->view('/'.X1.'/stock/recommend_list.php', array('rc_list' => $rc_list, 'tab' => '1'), true);
        echo $content_html;
        return;
    }
    ...
    public function recommend_view() {
		...
		if($rc_data['rc_endtype'] !='SUCCESS' && $rc_data['rc_endtype'] !='SELL') {
			...
			if($rc_data['rc_id'] == $free_ticker[0] || $rc_data['rc_id'] == $free_ticker[1] || $rc_data['rc_id'] == $free_ticker[2]) {
				//로그인체크 생략
			}
			else {
...
//print_r($data['mrt_list']);
        ...
        $this->header_data['back_url'] = '/'.X1.'_stock/recommend';
    ...
    public function recipe($type='dividend') {
        ...
        $data['score_content_html'] = $this->load->view('/'.X1.'/stock/recipe_list.php', array('type' => $type, 'list' => $list, 'is_open' => $this->is_open), true);
		$data['type'] = $type;
        ...
        $this->header_data['header_template'] = '6';
        $this->header_data['head_title'] = '투자 레시피';
        $this->header_data['back_url'] = '/'.X1.'_stock/recipe_intro';

		$data['meta_title'] = $data['title'].' | 투자레시피 - 초이스스탁US';

		if($this->session->userdata('is_paid') === false && $type == 'dividend') {
    ...
    public function ajax_get_recipe_list() {
        ...
        $content_html = $this->load->view('/'.X1.'/stock/recipe_list.php', array('type' => $type, 'list' => $list, 'is_open' => $this->is_open), true);
        echo $content_html;
        return;
    }
    ...
    private function _get_recipe_data($type) {
		...
		if($type == 'earnings') {
			...
			if($page == 1 && sizeof($recent_report_file)>0) {
				...
				$extra = array(
		            'fields' => array('tkr_ticker', 'tkr_name', 'tkr_currency', 'tkr_sf1_currency', 'tkr_rate', 'tkr_rate_str', 'tkr_close', 'tkr_exchange', 'sf1_netinc', 'sf1_netinccmnusd', 'dly_marketcap', 'sf1_lastupdated'),
    ...
    public function ajax_get_vod_list() {
        ...
        echo $this->load->view('/'.X1.'/stock/vod_mjm_list', $list_data, true);
    }
    ...
    public function vod_view($no=0) {
        ...
        if($row['vm_view_pe'] != 'Y' || $row['vm_display_date'] > date('Y-m-d H:i:s')) {
            $this->common->historyback();
            return;
        }
        ...
        $this->header_data['head_title'] = '동영상';
        $this->header_data['back_url'] = '/'.X1.'_stock/vod';
    ...
    public function ajax_get_research_list($more) {
        ...
        echo $this->load->view('/'.X1.'/stock/research_list', $list_data, true);
    }
    ...
    public function research_view($no=0) {
        ...
        if($row['e_view_pe'] != 'Y' || $row['e_display_date'] > date('Y-m-d H:i:s')) {
            $this->common->historyback();
            return;
        }
		...
		if($row['e_class']=='1') {
			$this->header_data['head_title'] = '미국주식 탐구생활';
			$this->header_data['back_url'] = '/'.X1.'_stock/research';
			$data['meta_title'] = $row['e_title'].' | 탐구생활 - 초이스스탁US';
			$data['back_url'] = '/'.X1.'_stock/research';
		}
		else {
			$this->header_data['head_title'] = '미국주식 종목노트';
			$this->header_data['back_url'] = '/'.X1.'_stock/note';
			$data['meta_title'] = $row['e_title'].' | 종목노트 - 초이스스탁US';
			$data['back_url'] = '/'.X1.'_stock/note';
		}
    ...
    public function ajax_get_morning_list() {
		...
		$list_data['is_event'] = $this->is_event;
        echo $this->load->view('/'.X1.'/stock/morning_list', $list_data, true);
    }
    ...
    public function morning_view($no=0) {
        ...
        if($row['e_display_date'] > date('Y-m-d H:i:s') || $row['e_view_pe'] != 'Y' || $row['e_class'] != '3') {
            $this->common->historyback();
            return;
        }
        ...
        $this->header_data['head_title'] = '모닝브리핑';
        $this->header_data['back_url'] = '/'.X1.'_stock/morning';
        //$this->header_data['back_url'] = '/';
	...
	public function push_control($push='') {
        ...
        if( in_array($push, array('Y', 'N'))) {
			...
			$this->common->locationhref('/'.X1.'_stock/view_close');
			exit;
		}
		else {
		...
		if( is_file($file_path) ) {
			...
			foreach($ticker_list as $ticker) {
				...
				if($ticker_info[$ticker['tkr_ticker']]['is_recom_ticker']) {
					$ticker_info[$ticker['tkr_ticker']]['recom_link'] = str_replace('/stock/recommend_view', '/'.X1.'_stock/recommend_view', $ticker_info[$ticker['tkr_ticker']]['recom_link']);
				...
				if($ticker_info[$ticker['tkr_ticker']]['mri']['m_biz_dividend_score'] >= 16) {
					$recipe = '<li><a href="/'.X1.'_stock/recipe/dividend">배당주</a></li>'."\n";
				}
				if($ticker_info[$ticker['tkr_ticker']]['mri']['m_future_score'] >= 16) {
					$recipe .= '<li><a href="/'.X1.'_stock/recipe/growth">성장주</a></li>'."\n";
				}
				if($ticker_info[$ticker['tkr_ticker']]['mri']['m_biz_moat_score'] >= 16) {
					$recipe .= '<li><a href="/'.X1.'_stock/recipe/moat">가치주</a></li>'."\n";
				}
                ...
                $ru = urlencode(X1.'_stock/catch_info');
				...
				if($choice_ticker != '' && $choice_name != '') {
					$industry_commnet = '<span class="same_selt"><a href="/'.X1.'_search/summary/'.$choice_ticker.'">'.$choice_name.'</a>와(과) 같은 업종입니다</span>';
				}
$pre = X1;
$catch_list = <<<PHPSKIN
...
PHPSKIN;
			...
			if($catch_all!='') {
				...
				echo $catch_all;
				if($is_next === true) {
					echo '<div class="next"><a href="/'.X1.'_stock/add_catch2?mode=auto&idx='.$idx.'&page='.$nextPage.'" class="nextPage">다음 페이지</a></div>';
				}
			}
		}
	}
				...
				foreach($ticker_list as $key => $ticker) {
					...
					if($ticker['tkr_ticker']!='') {
						...
						if(sizeof($recom_data) > 0) {
							...
							$ticker_info[$ticker['tkr_ticker']]['recom_link'] = '/'.X1.'_stock/recommend_view/'.$recom['rc_id'];
						}
						...
						*/
						...
						if($choice_ticker != '' && $choice_name != '') {
							$industry_commnet = '<span class="same_selt"><a href="/'.X1.'_search/invest_charm/'.$choice_ticker.'">'.$choice_name.'</a>와(과) 같은 업종입니다</span>';
						}
$pre = X1;
$catch_list = <<<PHPSKIN
<div class="chart_catch">
...
PHPSKIN;
				...
				if($catch_all!='') {
					...
					echo $catch_all;
					if($is_next === true) {
						echo '<div class="next"><a href="/'.X1.'_stock/add_catch?mode=auto&idx='.$idx.'&page='.$nextPage.'" class="nextPage">다음 페이지</a></div>';
					}
				}
			}
		}
...
//echo '<pre>'; print_r($data); exit;
	...
	public function catch_process() {
		...
		if( is_array($arr_type) && sizeof($arr_type) > 0 && is_array($arr_size) && sizeof($arr_size) > 0 && is_array($arr_sector) && sizeof($arr_sector) > 0 ) {
			...
			$this->common->locationhref('/'.X1.'_stock/catch_info');
			exit;

		}

		$this->common->locationhref('/'.X1.'_stock/catch_info?catch_list='.$c_tickerlist.'&sector_list='.$c_sector);
		exit;
	}
