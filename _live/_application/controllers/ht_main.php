...
class Ht_main extends Ht_BaseMobile_Controller {
		...
		if( is_file($file_path) ) {
			...
			foreach($morning_list as $morning) {
				if($morning['e_display_date'] <= date('Y-m-d H:i:s')) {
					...
					$cont_banner[$cont]['url'] = '/'.HT.'_stock/research_view/'.$morning['e_id'];
...
//echo '<pre>'; print_r($tab_stock_data);
    ...
    public function saveTicker() {
        ...
        $this->common->locationhref('/'.HT.'_main/search');
        exit;
    }

	public function service_info() {
        $this->load->view('/'.HT.'/main/service_info');
    }
}
