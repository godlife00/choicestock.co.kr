...
abstract class Ht_BaseMobile_Controller extends CI_Controller {
    ...
    function __construct() {
        ...
        //장 시작시간 체크 
        if(!$this->session->userdata('is_hts')){
	        $this->_open_check();
		}
		//echo '<pre>'; print_r($this->session->all_userdata());
		//https://hantoo.choicestock.co.kr/ht_main?serviceYN=Y&customNo=SUY4UUs3M1JKTmhSTFREL0NVa1F2dz09
    }
...
//서비스소개 : "https://hantoo.choicestock.co.kr/ht_main/service"  + &serviceYN="Y 또는 N" &customNo="고객번호"
//홈 :  "https://hantoo.choicestock.co.kr/ht_main" + &serviceYN="Y 또는 N" &customNo="고객번호"
//customNo : MjEwNDI3MDAwMDE=
        $pt_session = $this->input->get('serviceYN');
        $pt_usernum = $this->input->get('customNo');
        $pt_device  = $this->input->get('device');
		//echo base64_decode($pt_usernum);
		...
		if($pt_device != '' && $pt_device == '1') {
            $this->session->set_userdata('is_hts', TRUE);
		}

		if($pt_device != '' && $pt_device == '2') {
            $this->session->set_userdata('is_order', TRUE);
		}
		//echo $pt_usernum.'<br><br>';
		//echo base64_decode($pt_usernum); exit;
		//serviceYN=Y&customNo=%EA%B3%A0%EA%B0%9D%EC%8B%9D%EB%B3%84%EB%B2%88%ED%98%B8
    ...
    protected function payCheck(){
        if(!$this->session->userdata('is_paid')){
			...
			//echo '}';			
			//echo '</script>';
            $this->common->locationhref('/ht_main');
            exit;
		}
    }
...
/* remove
                ...
                case 'member/notice':
                    ...
                    break;
remove */
                case HT.'_main/myticker':
                    $show_menu = false;
                case HT.'_main/onestop':
                case HT.'_main/search':
                    $container_class = 'sub_search schfix_inc';
                    $show_type = '2';
                    $is_searching = true;
                    $searching_title = '진단';
                    break;
                case HT.'_stock/recommend':
                    $header['header_class'] = ' h_sub';
                ...
                case HT.'_stock/recommend_view':
                    ...
                    $is_sicon = false;
                    break;
                case HT.'_stock/analysis':
                    $header['header_class'] = ' h_sub';
                    $container_class = 'sub_analysis';
                    $show_type = '2';
                    //$is_vt_banner = TRUE;
                    break;
                case HT.'_stock/analysis_view':
                    $container_class = 'sub_analysis';
                    $show_type = '2';
                    break;
                case HT.'_stock/winner':
                    $header['header_class'] = ' h_sub';
                ...
                case HT.'_stock/vod_view':
                    ...
                    $soft_menu = 'attractive';
                    break;
                case HT.'_search/news_view':
                case HT.'_stock/research_view':
                    $container_class = 'sub_research';
                    $show_type = '2';
                    $is_headtop = true;
                    $headtop_title = '탐구생활';
                    ...
                    $soft_menu = 'attractive';
                    break;
                case HT.'_stock/morning_view':
                case HT.'_stock/morning':
                ...
                case HT.'_stock/catch_info':
                    ...
                    $searching_title = '발굴';
                    break;
                case HT.'_search/primary_ticker':
                case HT.'_search/news':
                case HT.'_search/fairvalue':
                case HT.'_search/invest_charm':
                case HT.'_search/summary':
                case HT.'_search/finance_chart':
                case HT.'_search/invest':
                case HT.'_search/financials':
                case HT.'_search/dividend':
                case HT.'_search/competitor':
                    $show_type = '2';
                    ...
                    $soft_menu = 'recommend';
                    break;
                case HT.'_attractiveness/attractive':
                    $header['header_class'] = ' h_sub';
                    $show_type = '2';
                ...
                case HT.'_main/menu':
                    ...
                    break;
                case HT.'_main/service':
                case HT.'_main/service_guide':
                case HT.'_main/service_prm':
                    $show_type = '3';
                    //$is_vt_banner = TRUE;
...
//remove        case 'payment/pay_free':
		...
		*/
        ...
        if( ! $return_contents) {
            ...
            $this->load->view('/'.HT.'/inc/header.php', $header);
            $this->load->view('/'.HT.'/'.$view.'.php', $data);
            $this->load->view('/'.HT.'/inc/footer.php', $footer);
        } else {

            //if($data['chkUri']!='hantoo') {
            $result  = $this->load->view('/'.HT.'/inc/header.php', $header, true)."\n";
            $result .= $this->load->view('/'.HT.'/'.$view.'.php', $data, true)."\n";
            //}
            $result .= $this->load->view('/'.HT.'/inc/footer.php', $footer, true)."\n";
            return $result;
        }
    }
