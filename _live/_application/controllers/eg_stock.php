...
class Eg_stock extends Eg_BaseMobile_Controller {
    ...
    public function index() {
        //$this->common->locationhref('/eg_stock/choice_api');
    ...
    public function choice_api($type='') {
        ...
        if($type=='A') {
            ...
            */
            ...
            $result[0]['link'] = '/'.EG.'_stock/recommend';
            ...
            $result[1]['link'] = '/'.EG.'_stock/recommend_view/'.$recommend[0]['rc_id'];
            ...
            $result[2]['link'] = '/'.EG.'_attractiveness/attractive?sort=total&netincome=all&marketcap=over100billion';
...
*/
        ...
        if($tab == '1') {
            $data['content_html'] = $this->load->view('/'.EG.'/stock/recommend_list.php', array('rc_list' => $rc_list, 'tab' => '1', 'is_event' => $this->is_event), true);
    ...
    public function ajax_get_recommend_list() {
        ...
        $content_html = $this->load->view('/'.EG.'/stock/recommend_list.php', array('rc_list' => $rc_list, 'tab' => '1'), true);
        echo $content_html;
        return;
    }
    ...
    public function recommend_view() {
        ...
        $rc_data['rc_invest_point'] = str_replace('https://www.choicestock.co.kr/','https://'.$host.'.choicestock.co.kr/'.EG.'_',$rc_data['rc_invest_point']);
...
//print_r($data['mrt_list']);
        ...
        $this->header_data['back_url'] = '/'.EG.'_stock/recommend';
    ...
    public function recipe($type='dividend') {
        ...
        $data['score_content_html'] = $this->load->view('/'.EG.'/stock/recipe_list.php', array('type' => $type, 'list' => $list, 'is_open' => $this->is_open), true);

		$data['type'] = $type;

        $this->header_data['header_template'] = '6';
        $this->header_data['head_title'] = '투자 레시피';
        $this->header_data['back_url'] = '/'.EG.'_stock/recipe_intro';
    ...
    public function ajax_get_recipe_list() {
        ...
        $content_html = $this->load->view('/'.EG.'/stock/recipe_list.php', array('type' => $type, 'list' => $list, 'is_open' => $this->is_open), true);
        echo $content_html;
        return;
    }
    ...
    public function ajax_get_vod_list() {
        ...
        echo $this->load->view('/'.EG.'/stock/vod_mjm_list', $list_data, true);
    }
    ...
    public function vod_view($no=0) {
        ...
        $this->header_data['head_title'] = '동영상';
        $this->header_data['back_url'] = '/'.EG.'_stock/vod';
    ...
    public function ajax_get_research_list($more) {
        ...
        echo $this->load->view('/'.EG.'/stock/research_list', $list_data, true);
    }
    ...
    public function research_view($no=0) {
        ...
        $row['e_content'] = str_replace('https://www.choicestock.co.kr/','https://'.$host.'.choicestock.co.kr/'.EG.'_',$row['e_content']);
		...
		if($row['e_class']=='1') {
			$this->header_data['head_title'] = '미국주식 탐구생활';
			$this->header_data['back_url'] = '/'.EG.'_stock/research';
			$data['meta_title'] = $row['e_title'].' | 탐구생활 - 초이스스탁US';
			$data['back_url'] = '/'.EG.'_stock/research';
		}
		else {
			$this->header_data['head_title'] = '미국주식 종목노트';
			$this->header_data['back_url'] = '/'.EG.'_stock/note';
			$data['meta_title'] = $row['e_title'].' | 종목노트 - 초이스스탁US';
			$data['back_url'] = '/'.EG.'_stock/note';
		}
    ...
    public function ajax_get_morning_list() {
        ...
        $list_data['is_event'] = $this->is_event;
        echo $this->load->view('/'.EG.'/stock/morning_list', $list_data, true);
    }
    ...
    public function morning_view($no=0) {
		...
		*/
        ...
        $this->header_data['head_title'] = '모닝브리핑';
        $this->header_data['back_url'] = '/'.EG.'_stock/morning';
        //$this->header_data['back_url'] = '/';
		...
		if( is_file($file_path) ) {
			...
			foreach($ticker_list as $ticker) {
				...
				if($ticker_info[$ticker['tkr_ticker']]['is_recom_ticker']) {
					$ticker_info[$ticker['tkr_ticker']]['recom_link'] = str_replace('/stock/recommend_view', '/'.EG.'_stock/recommend_view', $ticker_info[$ticker['tkr_ticker']]['recom_link']);
				...
				if($ticker_info[$ticker['tkr_ticker']]['mri']['m_biz_dividend_score'] >= 16) {
					$recipe = '<li><a href="/'.EG.'_stock/recipe/dividend">배당주</a></li>'."\n";
				}
				if($ticker_info[$ticker['tkr_ticker']]['mri']['m_future_score'] >= 16) {
					$recipe .= '<li><a href="/'.EG.'_stock/recipe/growth">성장주</a></li>'."\n";
				}
				if($ticker_info[$ticker['tkr_ticker']]['mri']['m_biz_moat_score'] >= 16) {
					$recipe .= '<li><a href="/'.EG.'_stock/recipe/moat">가치주</a></li>'."\n";
				}
                ...
                $ru = urlencode(EG.'_stock/catch_info');
				...
				if($choice_ticker != '' && $choice_name != '') {
					$industry_commnet = '<span class="same_selt"><a href="/'.EG.'_search/summary/'.$choice_ticker.'">'.$choice_name.'</a>와(과) 같은 업종입니다</span>';
				}
$pre = EG;
$catch_list = <<<PHPSKIN
...
PHPSKIN;
			...
			if($catch_all!='') {
				...
				echo $catch_all;
				if($is_next === true) {
					echo '<div class="next"><a href="/'.EG.'_stock/add_catch2?mode=auto&idx='.$idx.'&page='.$nextPage.'" class="nextPage">다음 페이지</a></div>';
				}
			}
		}
	}
                ...
                foreach($ticker_list as $key => $ticker) {
                    ...
                    if($ticker['tkr_ticker']!='') {
                        ...
                        if(sizeof($recom_data) > 0) {
                            ...
                            $ticker_info[$ticker['tkr_ticker']]['recom_link'] = '/'.EG.'_stock/recommend_view/'.$recom['rc_id'];
                        }
...
PHPSKIN;
                ...
                if($catch_all!='') {
                    ...
                    echo $catch_all;
                    if($is_next === true) {
                        echo '<div class="next"><a href="/'.EG.'_stock/add_catch?mode=auto&idx='.$idx.'&page='.$nextPage.'" class="nextPage">다음 페이지</a></div>';
                    }
                }
            }
        }
...
//echo '<pre>'; print_r($data); exit;
	...
	public function catch_process() {
		...
		if( is_array($arr_type) && sizeof($arr_type) > 0 && is_array($arr_size) && sizeof($arr_size) > 0 && is_array($arr_sector) && sizeof($arr_sector) > 0 ) {
			...
			$this->common->locationhref('/'.EG.'_stock/catch_info');
			exit;

		}

		$this->common->locationhref('/'.EG.'_stock/catch_info?catch_list='.$c_tickerlist.'&sector_list='.$c_sector);
		exit;
	}
