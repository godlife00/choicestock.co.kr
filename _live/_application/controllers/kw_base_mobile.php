...
//customNum : SUY4UUs3M1JKTmhSTFREL0NVa1F2dz09
		...
		if(($media == 'heros4' || $media == 'hero4') && $this->session->userdata('is_newlink') === false) {
            $this->session->set_userdata('is_newlink', TRUE);
		}
    ...
    protected function payCheckNew(){
        if(!$this->session->userdata('is_paid')){
			echo '<script>';
			echo 'var strJSONvalue = JSON.stringify(';
			echo '	{';
			echo '		\'command\' : \'activityGoto\',';
			echo '		\'param\' : { \'screen_id\' : \'2905\' },';
			echo '		\'callback\' : \'\',';
			echo '	}';
			echo ');';
			echo 'var agt = navigator.userAgent.toLowerCase();';
			echo 'if (agt.indexOf("iphone") != -1) {';
			echo '	window.webkit.messageHandlers.JSCallBack.postMessage(strJSONvalue);';
			echo '} else if (agt.indexOf("android") != -1) {';
			echo '	window.JSCallBack.postMessage(strJSONvalue);';
			echo '} else {';
			echo '	window.JSCallBack.postMessage(strJSONvalue);';
			echo '}';
			echo '</script>';
    ...
    protected function payCheck(){
		if($this->session->userdata('is_newlink')===TRUE) {
			$this->payCheckNew();
			return;
		}
...
remove */
                ...
                case KW.'_search/news':
                case KW.'_search/fairvalue':
