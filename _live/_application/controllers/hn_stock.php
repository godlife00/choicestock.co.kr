...
//print_r($data['mrt_list']);
    ...
    public function recipe($type='dividend') {
        ...
        $data['score_content_html'] = $this->load->view('/'.HN.'/stock/recipe_list.php', array('type' => $type, 'list' => $list, 'is_open' => $this->is_open), true);
        $data['type'] = $type;
    ...
    public function ajax_get_recipe_list() {
        ...
        $content_html = $this->load->view('/'.HN.'/stock/recipe_list.php', array('type' => $type, 'list' => $list, 'is_open' => $this->is_open), true);
        echo $content_html;
        return;
    }
    ...
    public function ajax_get_vod_list() {
        ...
        echo $this->load->view('/'.HN.'/stock/vod_mjm_list', $list_data, true);
    }
    ...
    public function vod_view($no=0) {
        ...
        $this->header_data['head_title'] = '동영상';
        $this->header_data['back_url'] = '/'.HN.'_stock/vod';
    ...
    public function research_view($no=0) {
		...
		if($row['e_class']=='1') {
			...
			$data['back_url'] = '/'.HN.'_stock/research';
		}
		else {
			$this->header_data['head_title'] = '미국주식 종목노트';
			$this->header_data['back_url'] = '/'.HN.'_stock/note';
			$data['meta_title'] = $row['e_title'].' | 종목노트 - 초이스스탁US';
			$data['back_url'] = '/'.HN.'_stock/note';
		}
		...
		if( is_file($file_path) ) {
			...
			foreach($ticker_list as $ticker) {
				...
				if($ticker_info[$ticker['tkr_ticker']]['is_recom_ticker']) {
					$ticker_info[$ticker['tkr_ticker']]['recom_link'] = str_replace('/stock/recommend_view', '/'.HN.'_stock/recommend_view', $ticker_info[$ticker['tkr_ticker']]['recom_link']);
				...
				if($choice_ticker != '' && $choice_name != '') {
					$industry_commnet = '<span class="same_selt"><a href="/'.HN.'_search/summary/'.$choice_ticker.'">'.$choice_name.'</a>와(과) 같은 업종입니다</span>';
				}
$pre = HN;
$catch_list = <<<PHPSKIN
...
PHPSKIN;
			...
			if($catch_all!='') {
				...
				echo $catch_all;
				if($is_next === true) {
					echo '<div class="next"><a href="/'.HN.'_stock/add_catch2?mode=auto&idx='.$idx.'&page='.$nextPage.'" class="nextPage">다음 페이지</a></div>';
				}
			}
		}
	}
...
//echo '<pre>'; print_r($data); exit;
	...
	public function catch_process() {
		...
		if( is_array($arr_type) && sizeof($arr_type) > 0 && is_array($arr_size) && sizeof($arr_size) > 0 && is_array($arr_sector) && sizeof($arr_sector) > 0 ) {
			...
			$this->common->locationhref('/'.HN.'_stock/catch_info');
			exit;
