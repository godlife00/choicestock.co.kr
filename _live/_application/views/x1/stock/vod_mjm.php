            ...
            <div class="sub_mid research_board">
				<div class="video_tabs">
					<a href="/<?=X1?>_stock/note" class="">종목노트</a>                    
					<a href="/<?=X1?>_stock/research" class="">탐구생활</a>                    
					<a href="/<?=X1?>_stock/vod" class="active">동영상</a>                    
				</div>
				...
				<div  id="analysis_list" class="lst_type video_list">
					...
					<div class="video_box<?=($val['vm_pay']=='Y' && $this->session->userdata('is_paid')===FALSE) ? ' lst_lock':''?>">
						...
						<a href="javascript:fnLogin();">
						<?php else :?>
						<a href="/<?=X1?>_stock/vod_view/<?=$val['vm_id']?>">
						<?php endif;?>												
            ...
            function view_more() {
                ...
                page += 1;
                $.post('/<?=X1?>_stock/ajax_get_vod_list', {'page': page}, function(res) {
                    if($.trim(res).length) {
