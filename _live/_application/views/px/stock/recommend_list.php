                                ...
                                <div class="chart_area">
                                    <div class="chartData">
                                        <div class="left">
											...
											<a href="javascript:fnPay();"><?=iconv_substr($r['ticker']['tkr_name'], 0, 1, 'utf-8')?><span class="remark"><div class="txt_filter size_M"><i></i><i></i><i></i><i></i></div></span>
											<?php else :?>
											<a href="/<?=PX?>_stock/recommend_view/<?=$r['rc_id']?>"><?=$r['ticker']['tkr_name']?>
											<?php endif;?>
											</a></h4>
                                            <ul class="info">
												...
												<?php if($this->session->userdata('is_paid')===FALSE) :?>
												<a href="javascript:fnPay();">
												<?php else :?>
												<a href="/<?=PX?>_stock/recommend_view/<?=$r['rc_id']?>">
												<?php endif;?>												
                                    ...
                                    <div class="chartGoal">
                                        <dl>
											...
											<?php //else :?>
											<!--<dd class="prm_lock"><img src="/img/prm_lock_red.png" alt="잠김"></dd>-->
											<?php //endif;?>
                                        </dl>
                                        <dl>
										...
										<span class="attainment attainment_com"><span>목표가달성</span> <i class="<?=($r['rc_profit_rate']>0) ? 'increase':'decrease'?>">+<?=number_format($r['rc_profit_rate'],2)?><b>%</b></i></span>
										<?php elseif($r['rc_endtype'] == 'FAIL') :?>
										<span class="attainment attainment_com loss_cut"><span>손절가매도 </span> <i class="<?=($r['rc_profit_rate']>0) ? 'increase':'decrease'?>"><?=($r['rc_profit_rate']>0) ? '+':''?><?=number_format($r['rc_profit_rate'],2)?><b>%</b></i></span>
										<?php elseif($r['rc_endtype'] == 'SELL') :?>
										<span class="attainment attainment_com"><span>교체매도 </span> <i class="<?=($r['rc_profit_rate']>0) ? 'increase':'decrease'?>"><?=($r['rc_profit_rate']>0) ? '+':''?><?=number_format($r['rc_profit_rate'],2)?><b>%</b></i></span>
										<?php else :?>
