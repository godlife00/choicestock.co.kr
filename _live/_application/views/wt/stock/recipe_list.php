<?php if($is_open === true && $type != 'soaring'):?>
<script type="text/javascript">
<!--
<?php if($type == 'earnings') :?>
	<?php $i=0; foreach($list['recent_report'] as $val) :?>
...
<?php else :?>
	<?php $i=0; foreach($list as $val) :?>
		var mi_pre<?=$i?> = <?=$val['m_close']?>;
	<?php $mi_tickers .= $val['m_ticker'].'|';?>
	<?php $i++; endforeach;?>
<?php endif;?>
...
<?php endif;?>
								...
								<?php if($type == 'earnings') :?>
                                    <?php $cnt=0; foreach($list['recent_report'] as $key => $val) : ?>
										<div class="area">
											<ul class="list">
												<li class="title"><a href="/<?=WT?>_search/summary/<?=$val['tkr_ticker']?>"><?=$val['tkr_name']?></a></li>
												<li class="num">
                                    ...
                                    <?php $cnt++; endforeach;?>
								<?php elseif($type == 'interest') :?>
									<?php $cnt=0; foreach($list as $val) :?>
									<div class="area">
										<ul class="list">
											<li class="title">
												<a href="/<?=WT?>_search/summary/<?=$val['m_ticker']?>"><?=$val['ticker']['tkr_name']?></a>
											...
											<li class="attention_box">
												<span class="attention_score"><?=$val['m_biz_total_score']?>P</span>
											</li>
										</ul>
										<ul class="analysis_score">
								...
								<?php elseif($type == 'sale') :?>
									...
									<div class="area">
										<ul class="list">
											...
											<?php if($this->session->userdata('is_paid')===TRUE) :?>
												<a href="/<?=WT?>_search/summary/<?=$val['tkr_ticker']?>"><?=$val['tkr_name']?></a>
												<span class="sum"><?=$val['tkr_ticker']?></span>
											<?php else :?>										
												<a href="#" data-modal="modal-3" class="btn_free md-trigger"><?=iconv_substr($val['tkr_name'], 0, 1, 'utf-8')?><span class="remark"><div class="txt_filter size_M"><i></i><i></i><i></i><i></i></div></span></a>
												<span class="sum">
									...
									<?php $cnt++; endforeach;?>
                                <?php else :?>
                                    <?php $cnt=0; foreach($list as $key => $val) : ?>
									<?php 
										$stars = 0;
										$price = 0;
										if($type == 'dividend') {
									...
									<div class="area">
										<ul class="list">
											...
											<?php else :?>										
												<li class="title"><a href="/<?=WT?>_search/summary/<?=$val['m_ticker']?>"><?=$val['m_korname']?></a></li>
											<?php endif;?>
											<li class="num">
												<span><?=$price?><b>%</b></span>
											</li>
