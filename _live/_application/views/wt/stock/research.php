            ...
            <div class="sub_top">
                <div class="">
                    <div class="txt_box">
                        <p class="txt">실적 및 배당 발표 기업과 시장에서 관심있는 테마에 대한 종목 리스트를 제공합니다.
            ...
            <div class="sub_mid research_board">
                ...
                <div class="video_tabs">
                    <a href="/<?=WT?>_stock/note"<?=($this_page == 'note') ? ' class="active"':''?>>종목노트</a>                    
                    <a href="/<?=WT?>_stock/research"<?=($this_page == 'research') ? ' class="active"':''?>>탐구생활</a>                    
					<?php if($is_google === FALSE ):?>
                    <a href="/<?=WT?>_stock/vod" class="">VIP 전략</a>                    
                    <a href="/<?=WT?>_stock/vod/edu" class="">길잡이</a>                    
					<?php endif;?>
                </div>

                <!--<div class="set">
                    <span onclick="location.href='/<?=WT?>_stock/<?=$this_page?>/display_date';" class="<?=$order_by == 'display_date' ? 'active' : ''?>">최신글</span>
                    <span onclick="location.href='/<?=WT?>_stock/<?=$this_page?>/view_count';" class="<?=$order_by == 'view_count' ? 'active' : ''?>">인기글</span>
                </div>-->
                <ul id="analysis_list" class="lst_type">
                    ...
                    <li>
                        <dl class="lst_type2<?=($exp['e_pay']=='Y' && $this->session->userdata('is_paid')===FALSE) ? ' lst_lock':''?>">
                            ...
                            <?php endif;?>
                            <strong><?=nl2br($exp['e_title'])?></strong></a><span class="reg_day"><?=date('y.m/d', strtotime($exp['e_display_date']))?></span></dt>
                            <dd class="photo">
			...
			*/
            ...
            function view_more() {
                ...
                page += 1;
                $.post('/<?=WT?>_stock/ajax_get_research_list/<?=$this_page?>', {'page': page, 'order_by':'<?=$order_by?>'}, function(res) {
                    if($.trim(res).length) {
