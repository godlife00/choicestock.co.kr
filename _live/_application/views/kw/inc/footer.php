		...
		<div class="cho_applybtn">
            <a href="javascript:<?=($this->session->userdata('is_newlink')===TRUE) ? 'fnSinChungNew();':'fnSinChung();'?>" class="btn btn_apply">미주미초이스 신청하기<i></i></a>
        </div>
        <!-- //cho_applybtn -->
		<?php endif;?>
    </div>
                    ...
                    switch(n.type) {
                        ...
                        case 'diff_rate' :
                            if(callback_resp[ticker] != null) {
                                ...
                                //n.target.html(rate);
                                n.target.stop().animate({'background-color':'#ddd'}, 500).html(rate).animate({'background-color':'transparent'}, 500);
                            }
                            break;
                        ...
                        case 'diff_price' :
                            if(callback_resp[ticker] != null) {
                                ...
                                n.target.stop().animate({'background-color':'#ddd'}, 500).html(diff_price).animate({'background-color':'transparent'}, 500);
                            }
                            break;
