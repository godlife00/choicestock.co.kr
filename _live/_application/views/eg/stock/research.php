                    ...
                    <div class="swiper-wrapper">
                        ...
                        <div class="swiper-slide">
                            <div class="best_research">
                                ...
                                <?php else :?>
                                <a href="/<?=EG?>_stock/research_view/<?=$val['e_id']?>">
                                <?php endif;?>
            ...
            <div class="sub_mid research_board">
                <div class="video_tabs">
                    <a href="/<?=EG?>_stock/note"<?=($this_page == 'note') ? ' class="active"':''?>>종목노트</a>                    
                    <a href="/<?=EG?>_stock/research"<?=($this_page == 'research') ? ' class="active"':''?>>탐구생활</a>                    
                    <a href="/<?=EG?>_stock/vod" class="">동영상</a>                    
                </div>
                <div class="set">
                    <span onclick="location.href='/<?=EG?>_stock/<?=$this_page?>/display_date';" class="<?=$order_by == 'display_date' ? 'active' : ''?>">최신글</span>
                    <span onclick="location.href='/<?=EG?>_stock/<?=$this_page?>/view_count';" class="<?=$order_by == 'view_count' ? 'active' : ''?>">인기글</span>
                </div>
            ...
            function view_more() {
                ...
                page += 1;
                $.post('/<?=EG?>_stock/ajax_get_research_list/<?=$this_page?>', {'page': page, 'order_by':'<?=$order_by?>'}, function(res) {
                    if($.trim(res).length) {
