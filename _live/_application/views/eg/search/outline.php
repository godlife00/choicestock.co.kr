...
<?php if($sec_ticker=='' && $pri_ticker=='') {?>
				<div class="sub_mid outline_box">
					<a href="/<?=EG?>_search/invest_charm/<?=$ticker_code?>" class="box">
					...
					<?php if($fairvalue_rate >= -5) :?>
						...
						<a href="/<?=EG?>_search/fairvalue/<?=$ticker_code?>" class="box">
				...
				<?php endif;?>
					...
					<a href="/<?=EG?>_search/invest/<?=$ticker_code?>" class="box">
					...
					<div class="box">
						<p class="title"><a href="<?=($this->session->userdata('is_paid')===TRUE || $open_ticker===TRUE)? '/'.EG.'_stock/recipe_intro':'javascript:;'?>">투자레시피</a></p>
						<div class="con">
							<div class="event_attractiveness">
								<ul>
									<?php $at_count = 0; if($mri_data['m_biz_dividend_score'] >= 16) : ?>
									<li class="att_on"><a href="<?=($this->session->userdata('is_paid')===TRUE || $open_ticker===TRUE)? '/'.EG.'_stock/recipe/dividend':'javascript:;'?>">배당매력주</a></li>
									<?php $at_count++; endif; ?>

									<?php if($mri_data['m_future_score'] >= 16) : ?>
									<li class="att_on"><a href="<?=($this->session->userdata('is_paid')===TRUE || $open_ticker===TRUE)? '/'.EG.'_stock/recipe/growth':'javascript:;'?>">이익성장주</a></li>
									<?php $at_count++; endif; ?>

									<?php if($mri_data['m_biz_moat_score'] >= 16) : ?>
									<li class="att_on"><a href="<?=($this->session->userdata('is_paid')===TRUE || $open_ticker===TRUE)? '/'.EG.'_stock/recipe/moat':'javascript:;'?>">소비자독점주</a></li>
									<?php $at_count++; endif; ?>
					...
					<?php if($this->session->userdata('is_paid')===TRUE || $open_ticker===TRUE) :?>
					<a href="/<?=EG?>_search/competitor/<?=$ticker_code?>" class="box">
					<?php else :?>
					<a href="javascript:fnSinChung();" class="box">
					<?php endif;?>
...
<?php }else{?>
		...
		<div class="sub_mid nondata">
			<p class="nodata_guide"><strong><?=$sec_ticker;?></strong>의 상세 기업정보는 <strong><?php if($part_page == ''){?><a href="/<?=EG?>_search/summary/<?=$pri_ticker?>?pn=<?=$part_name;?>&pg=<?=$part_page;?>"><?php }?><?=$pri_ticker?><?php if($part_page == ''){?></a><?php }?></strong> 종목에서 확인할 수 있습니다.</p>
		</div>
		<!-- //sub_mid nondata -->
<?php }?>
