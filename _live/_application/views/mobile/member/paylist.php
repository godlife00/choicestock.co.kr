			...
			<?php include_once INC_PATH.'/info_submenu.php'; ?>
                ...
                <div class="<?=($autopay['p_at_count']==0 || $autopay['p_at_count']=='') ? 'mapage_service_free':'mapage_service'?>">
                    ...
                    <div class="btm">
                        ...
                        <?php if($autopay['is_update']===TRUE) :?><a href="/payment/card_update/<?=$autopay['p_id']?>" class="card_ch">변경</a><?php endif;?>
                    </div>
					<?php else:?>
                        ...
                        <tbody>
                            <tr>
								...
								<?php
									...
									*/
									$dd = substr($autopay['p_start_date'], -2);
									$end_ym = substr($autopay['p_end_date'], 0, 6).$dd;
								?>
								<!--<?=date('Y.m/d', strtotime($start_ym))?> ~ <?=date('Y.m/d', strtotime($end_ym))?>-->
								<?=date('Y.m/d', strtotime($autopay['p_start_date']))?> ~ <?=date('Y.m/d', strtotime($end_ym))?>
								<?php else :?>
                            ...
                            <tr>
								...
								<?php if($autopay['p_card_code'] != 'PA' && $autopay['p_at_day'] != '') :?>
                                ...
                                <td class="num"><span>ㆍ카드번호</span> <strong>*********<?=substr($autopay['p_card_no'], -4)?></strong><?php if($autopay['is_update']===TRUE) :?><a href="/payment/card_update/<?=$autopay['p_id']?>" class="card_ch">변경</a><?php endif;?></td>
                            </tr>
                            <tr>
                ...
                <div class="mapage_service">
                    <table border="1" class="service_table">
                        <tbody>
                            <tr>
                                <th class="service">프리미엄 서비스 : </th>
								<?php $cnt = (strtotime($freepay_data['fp_end_date']) - strtotime($freepay_data['fp_start_date'])) / 86400;?>
                                <td class="day"><?=date('Y.m/d', strtotime($freepay_data['fp_start_date']))?> ~ <?=date('Y.m/d', strtotime($freepay_data['fp_end_date']))?> (<?=($cnt>3) ? '2주':'3일'?> 무료체험)</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
							...
							<table border="1" class="service_table">
								<tbody>
									<tr>
										...
										<td class="day"><?=date('Y.m/d', strtotime($paylist[0]['p_start_date']))?> ~ <?=date('Y.m/d', strtotime($paylist[0]['p_end_date']))?>
										<?php if(substr($paylist[0]['p_pt_code'], 0, 3) != 'CSM') :?>
										(<?=$pay_info[$paylist[0]['p_pay']]['month']?>개월)
										<?php endif;?>
										</td>
									</tr>
								</tbody>
						...
						<?php $s_date = strtotime(date('Ymd'));?>
							...
							<?php //무통장인데 결제예정일 벗어난 경우 continue;?>
								...
								<tr>
									...
									?>
									<td><?=number_format($str_price)?>원 <span class="in">
									<?php if($pay['p_type']=='A'):?>
									월자동 
									<?php else : ?>
										<?php if(substr($pay['p_pt_code'], 0, 3) != 'CSM') :?>
										<?=$pay_info[$pay['p_pay']]['month']?>개월
										<?php endif;?>
									<?php endif;?>
									</span> </td>
											...
											<tr>
												...
												<td>
													...
													<div class="certification_area">
														<p class="txt">
														<b>월 정기결제</b> : 결제 주기가 종료될 때까지 서비스를 이용할 수 있으며, 결제 금액은 환불되지 않습니다.<br><br>
														결제취소는 신청 즉시(또는 영업일 기준 D+2 이내) 처리되며, 가상계좌/계좌이체 결제금액은
															D+7 이내 등록하신 환불계좌로 입금처리됩니다.</p>
														<a href="https://www.datahero.co.kr/choicestockus-faq/" class="go_link" target="_blank"><i></i>환불정책 바로가기</a>
													</div>
													<!-- //certification_area -->
												</td>
											</tr>
										...
										<tbody>
											...
											<tr>
												...
												<td>
													...
													<div class="certification_area">
														<p class="txt">
														<b>기간결제</b> : 해지일로부터 이용일 수에 해당하는 금액과 수수료 및 제반비용(3.5%)차감후 환불됩니다.<br><br>
														결제취소는 신청 즉시(또는 영업일 기준 D+2 이내) 처리되며, 가상계좌/계좌이체 결제금액은
                                                        D+7 이내 등록하신 환불계좌로 입금처리됩니다.</p>
														<a href="https://www.datahero.co.kr/choicestockus-faq/" class="go_link" target="_blank"><i></i>환불정책 바로가기</a>
													</div>
