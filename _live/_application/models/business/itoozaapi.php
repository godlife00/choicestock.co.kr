...
class ItoozaApi extends BaseBusiness {
    ...
    public function getSEPListForChart($tickers=array(), $lifetime=23400) {
        ...
        foreach($data as $ticker => &$seps) {
            ...
            $closes = array();
            foreach($seps as $sep) {
                $closes[$sep['sep_date']] = $sep['sep_dividends'];
            }
            $seps = $closes;
        }
...
*/
    ...
    public function getCharmFinanceVChartFile($ticker_code, $dimension, $is_adr = false) {
		...
		$dimension_indi_map['MRQ']['psr'] = 'MRT';
        $dimension_indi_map['MRQ']['pricesps'] = 'MRT';
    ...
    public function getCharmFinanceVChart($ticker_code, $dimension, $lifetime=3600*24, $is_adr = false) {
    ...
    // 배당락일 가져오기(2021.03.15 추가)
    public function getExdate($ticker) {
        return $this->_dataGetContents('getExdate/'.$ticker);
    }

    // 배당 데이터 가져오기(21.03/29)
    public function getDividend($ticker_code) {
        return $this->_dataGetContents('getDividend/'.$ticker_code);
    }
}
