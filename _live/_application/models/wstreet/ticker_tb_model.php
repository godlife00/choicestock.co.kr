...
class Ticker_tb_model extends MY_Model {
    ...
    public function convertWowInfo($row) {
        if( ! isset($row['tkr_ticker'])) {
            return $row;
        }
		if(isset($row['is_wow']) && $row['is_wow'] == true) {
	        $item = array_shift(json_decode($this->common->get_wtsise($row['tkr_ticker']), true));
		}
		else {
			$item = $this->common->get_wowtv_info($row['tkr_ticker']);
		}
...
*/
        $row['tkr_close'] = $item['last_price'];
        $row['tkr_rate'] = $item['diff_rate'];
        $row['tkr_lastpricedate'] = ($item['status']=='0') ? date('Y-m-d H:i:s', strtotime($item['updated_at'])) : date('Y-m-d', strtotime($item['updated_at']));
        $row['tkr_rate_str'] = ($item['diff_rate']>0) ? '+'.$item['diff_rate'].'%':$item['diff_rate'].'%';
        $row['tkr_diff_str'] = ($item['diff_price']>0) ? '+ '.$item['diff_price']:$item['diff_price'];
        $row['tkr_status'] = $item['status'];
