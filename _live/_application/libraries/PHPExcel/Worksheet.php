...
class PHPExcel_Worksheet implements PHPExcel_IComparable
{
	...
	*/
	 ...
	 * @return PHPExcel_Worksheet
	 */
	public function fromArray($source = null, $nullValue = null, $startCell = 'A1', $strictNullComparison = false, $string_cells = array()) {
		if (is_array($source)) {
				...
				foreach($rowData as $cellValue) {
					if ($strictNullComparison) {
						if ($cellValue !== $nullValue) {
							// Set cell value
							if(in_array($currentColumn . $startRow, $string_cells)) {
						...
						if ($cellValue != $nullValue) {
							// Set cell value
							// 문자열 출력을 원할경우 string_cells에 해당 행열 정보가 있는지 체크 후 STRING으로 셋팅한다. 
							if(in_array($currentColumn . $startRow, $string_cells)) {
								$this->getCell($currentColumn . $startRow)->setValueExplicit($cellValue);
							} else {
								$this->getCell($currentColumn . $startRow)->setValue($cellValue);
