//신청/해지 New(21.10.26)
function fnSinChungNew() {
	var strJSONvalue = JSON.stringify(
        {
            'command' : 'activityGoto',
            'param' : { 'screen_id' : '2905' },
            'callback' : '',
        }
    );
    ...
    if (agt.indexOf("iphone") != -1) {
		window.webkit.messageHandlers.JSCallBack.postMessage(strJSONvalue);
    } else if (agt.indexOf("android") != -1) {
		window.JSCallBack.postMessage(strJSONvalue);
    } else {
		window.JSCallBack.postMessage(strJSONvalue);
    }
}
//서비스소개 New(21.10.26)
function fnServiceNew() {
	var strJSONvalue = JSON.stringify(	
			{	
				'key' : 'activityGoTo',
				'screen' : '2905'
...
function fnVod() {
    ...
    var agt = navigator.userAgent.toLowerCase();
    if (agt.indexOf("iphone") != -1) {
        window.location = "SuperStock://activityGoTo//vod";
    } else if (agt.indexOf("android") != -1) {
        window.SuperStock.activityGoTo("vod");
    } else {
        window.SuperStock.activityGoTo("vod");
    }
}
    ...
    $.ajax({
        ...
        success : function(data) {
            if(data.success) {
                ...
                location.href='/kw_stock/catch_info?catch_list='+data.catch_list+'&sector_list='+data.sector_list;
            }
            else {
                alert(data.msg);
                location.href='/kw_stock/catch_info';
            }
        }
    });
}
...
function fnCatchNewSave() {
	...
	$('#loading').css("display", "block");
	location.href='/kw_stock/catch_process?catchType='+catchType+'&catchSize='+catchSize+'&catchSector='+catchSec;
}
...
function fnMyTickerSave() {
    ...
    location.href= '/kw_main/saveTicker?ticker='+ticker;
}
...
function fnMyitem(ticker, page, count, id) {
    ...
    var href = '/kw_search/setTicker/'+ticker+'/'+count;
...
function moveToSearch(div) {
    ...
    if ($('li a._on', div).length > 0) {
        ...
        location.href = '/kw_search/summary/' + keyword;
    } else {
...
function onSearchTicker(ticker) {
    ...
    location.href = '/kw_search/summary/' + ticker;
}
...
function setSearchHistory(keyword) {
    ...
    */
    ...
    $.ajax({
        url: '/kw_main/ajax_save_search_history',
        type: 'GET',
        data: { 'ticker': keyword },
        async: false,
        success: function () {
