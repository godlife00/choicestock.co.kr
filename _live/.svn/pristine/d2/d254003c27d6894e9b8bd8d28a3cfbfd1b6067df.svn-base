...
*/
    ...
    public function getRecomStockList($type, $limit=5, $addParams=array(), $addExtra=array()) {
		...
		if($type == 'dividend') {
			$exc_dividend_file = 'exc_dividend.json';
			$file_path = str_replace('hoon','datahero',WEBDATA).'/json/'.$exc_dividend_file;

			if( is_file($file_path) ) {
				$exc_list = json_decode(file_get_contents($file_path), true);
				$params['not in']['m_ticker'] = $exc_list;
			}
		}
        ...
        switch($type) {
            ...
            case 'growth' : // 수익(이익)성장성
                ...
                $extra['order_by'] = 'm_future_score desc, m_biz_total_score desc';
                break;
            case 'safety' : // 재무안정성
