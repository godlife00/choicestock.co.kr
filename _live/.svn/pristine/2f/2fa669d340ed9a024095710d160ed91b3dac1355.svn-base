	...
	public function tp_parse($pName = "")
	{
		if ($pName == "")
		{
			...
			for($i = 0; $i < $count; $i+=2)
			{
				...
				if (is_array($this->TEMPLATE_VAR["main"][$data[$i+1]]))
					$this->TEMPLATE_CONTENT["main"] .=  implode("", $this->TEMPLATE_VAR["main"][$data[$i+1]]);
				else
					$this->TEMPLATE_CONTENT["main"] .= $this->TEMPLATE_VAR["main"][$data[$i+1]];
			}
		}
		else
		{

			if (!$this->TEMPLATE_IS_PARSE[$pName])
			{
			...
			for($i = 0; $i < $count; $i+=2)
			{
				...
				if (is_array($this->TEMPLATE_VAR[$pName][$this->TEMPLATE_CONTENT_DATA[$pName][$i+1]]))
				{
