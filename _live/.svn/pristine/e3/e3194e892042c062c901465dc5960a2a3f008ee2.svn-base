...
//print_r($data['mrt_list']);
    ...
    public function analysis() {
        ...
        $data['content_html'] = $this->load->view('/'.EG.'/stock/analysis_list', $list_data, true);
        $this->header_data['header_template'] = '2';
    ...
    public function ajax_get_analysis_list() {
        ...
        echo $this->load->view('/'.EG.'/stock/analysis_list', $list_data, true);
    ...
    public function recipe($type='dividend') {
        ...
        $data['score_content_html'] = $this->load->view('/'.EG.'/stock/recipe_list.php', array('type' => $type, 'list' => $list), true);
        $data['type'] = $type;
    ...
    public function ajax_get_recipe_list() {
        ...
        $content_html = $this->load->view('/'.EG.'/stock/recipe_list.php', array('type' => $type, 'list' => $list), true);
        echo $content_html;
        return;
    }
    ...
    public function push_control($push='') {
        ...
        if( in_array($push, array('Y', 'N'))) {
            ...
            $this->common->locationhref('/'.EG.'_stock/view_close');
            exit;
        }
        else {
            $this->common->locationhref('/');
                ...
                foreach($ticker_list as $key => $ticker) {
                    ...
                    if($ticker['tkr_ticker']!='') {
                        ...
                        if($choice_ticker != '' && $choice_name != '') {
                            $industry_commnet = '<span class="same_selt"><a href="/'.EG.'_search/invest_charm/'.$choice_ticker.'">'.$choice_name.'</a>와(과) 같은 업종입니다</span>';
                        }
$pre = EG;
$catch_list = <<<PHPSKIN
                ...
                foreach($ticker_list as $key => $ticker) {
                    ...
                    if($ticker['tkr_ticker']!='') {
                        ...
                        if(sizeof($analysis_data) > 0) {
                            ...
                            $ticker_info[$ticker['tkr_ticker']]['analysis_link'] = '/'.EG.'_stock/analysis_view/'.$analy['an_id'];
                        }
                        */
