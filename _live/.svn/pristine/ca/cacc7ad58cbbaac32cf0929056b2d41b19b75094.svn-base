            ...
            <div class="sub_mid research_board">
                <div class="set">
                    <span onclick="location.href='/<?=PX?>_stock/research/display_date';" class="<?=$order_by == 'display_date' ? 'active' : ''?>">최신글</span>
                    <span onclick="location.href='/<?=PX?>_stock/research/view_count';" class="<?=$order_by == 'view_count' ? 'active' : ''?>">인기글</span>
                </div>
            ...
            function view_more() {
                ...
                page += 1;
                $.post('/<?=PX?>_stock/ajax_get_research_list', {'page': page, 'order_by':'<?=$order_by?>'}, function(res) {
                    if($.trim(res).length) {
