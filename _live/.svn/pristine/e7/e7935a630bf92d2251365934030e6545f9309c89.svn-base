...
<!--InAppType : 0 -> 단품결제, 1->정기구독 -->
var payOrderNo = '';
function requestBuyProduct(inappType, productId){
	...
	var url = '/wt_member/and_order';
	var orderId = '';

	$.post(url, param, function(res) {
        if( ! res.is_success) {
			HK.showToast(res.error_message);
			return;
		}
		else {
			orderId = res.order_no;
			payOrderNo = res.order_no;
			...
			logWrite('requestBuyProduct::message::', message);

			HK.postMessage(JSON.stringify(message));
		}
	}, 'json');
}
...
function responseBuyProduct(message){
	...
	var jsonArrayData = JSON.parse(decodedData)
	console.log("[create] : [jsonArray] : " + JSON.stringify(jsonArrayData));
	console.log("[create] : [jsonArray]  LEN: " + jsonArrayData.length);

	var param = {
        'payResultCode' : jsonArrayData[0].payResultCode, 
        //'payOrderNo' : jsonArrayData[0].payOrderNo, 
        'payOrderNo' : payOrderNo, 
        'memberId' : jsonArrayData[0].memberId, 
        'packageId' : jsonArrayData[0].packageId, 
        'payTransNo' : jsonArrayData[0].payTransNo, 
        'payTransTime' : jsonArrayData[0].payTransTime, 
        'payToken' :  jsonArrayData[0].payToken, 
    };

	logWrite('sendVerifyReceiptResult[0]::param::', param);

	var url = '/wt_member/and_noti';
	...
	$.post(url, param, function(res) {
        if( ! res.is_success) {
			...
			sendVerifyReceiptResult(jsonArrayData);
		}
	}, 'json');
}

function sendVerifyReceiptResult(strData){
  var jsonArray = new Array() ;
  for(var i=0; i<strData.length; i++){ //배열 값 순차적으로 확인
    var jsonObject = JSON.parse(JSON.stringify(strData[i])); //각 배열에 있는 jsonObject 참조
    console.log("[parsing] : [GTKN] : " + jsonObject.GTKN);
    var resultData = 'result:'+'TRUE'+","+"token:"+jsonObject.GTKN
    var jsonObj = new Object()
    jsonObj.result = "TRUE"
    jsonObj.token = jsonObject.GTKN
  ...
  console.log("responseBuyProduct : "+JSON.stringify(message));
  HK.postMessage(JSON.stringify(message));
}
...
<!--<script type="text/javascript" src="/js/common_and_pay.js?v=<?=filemtime(APP_PATH.'/js/common_and_pay.js')?>"></script>-->
...
<body class="globalStock wowtv">
    <div id="wrap">
        ...
        <div id="container">
            ...
            <!--<a href="javascript:requestBuyProduct(2,'101802');" style="width: 100%"><img src="https://swimg.wowtv.co.kr/static/images/event_img/2022/220311_us_02.jpg" width="100%" border="0" alt="월 정기 정기 결제 할인"></a>-->
            <a href="javascript:HK.showToast('준비중입니다.');" style="width: 100%"><img src="https://swimg.wowtv.co.kr/static/images/event_img/2022/220311_us_02.jpg" width="100%" border="0" alt="월 정기 정기 결제 할인"></a>
            <img src="https://swimg.wowtv.co.kr/static/images/event_img/2022/220311_us_03.jpg" width="100%" border="0">
            <a href="javascript:requestBuyProduct(1, '101758');" style="width: 100%"><img src="https://swimg.wowtv.co.kr/static/images/event_img/2022/220311_us_04.jpg" width="100%" border="0" alt="1개월 결제하기" /></a>
			...
			*/
            ...
            <a href="javascript:requestBuyProduct(1, '101758');" style="width: 100%"><img src="https://swimg.wowtv.co.kr/static/images/event_img/2022/220817_us_02.jpg" width="100%" border="0"></a>
            <img src="https://swimg.wowtv.co.kr/static/images/event_img/2022/220817_us_03.jpg" width="100%" border="0">
        </div>
        <!-- //container -->
    </div>
    <!-- //wrap -->
