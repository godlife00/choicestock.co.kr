...
<?php if($sec_ticker=='' && $pri_ticker=='') {?>
				<div class="sub_mid outline_box">
					<a href="/<?=KW?>_search/invest_charm/<?=$ticker_code?>" class="box">
					...
					<?php if($fairvalue_rate >= -5) :?>
						...
						<a href="/<?=KW?>_search/fairvalue/<?=$ticker_code?>" class="box">
						...
						<?php else :?>
						<a href="javascript:<?=($this->session->userdata('is_newlink')===TRUE) ? 'fnSinChungNew();':'fnSinChung();'?>" class="box">
							<p class="title"><span>AI적정주가</span></p>
							<div class="con">
								<div class="value_chart">                                                    
									<div class="prm_lock"><span><i></i>미주미초이스 신청하기</span></div>
									<div class="line">                  
										<span class="g_action" style="left: 50%;"></span>                                              
									</div>
									<div class="adequate">
				...
				<?php endif;?>
					...
					<a href="/<?=KW?>_search/invest/<?=$ticker_code?>" class="box">
					...
					<div class="box">
						<p class="title"><a href="<?=($this->session->userdata('is_paid')===TRUE || $open_ticker===TRUE)? '/'.KW.'_stock/recipe_intro':'javascript:;'?>">투자레시피</a></p>
						<div class="con">
							<div class="event_attractiveness">
								<ul>
									<?php $at_count = 0; if($mri_data['m_biz_dividend_score'] >= 16) : ?>
									<li class="att_on"><a href="<?=($this->session->userdata('is_paid')===TRUE || $open_ticker===TRUE)? '/'.KW.'_stock/recipe/dividend':'javascript:;'?>">배당매력주</a></li>
									<?php $at_count++; endif; ?>

									<?php if($mri_data['m_future_score'] >= 16) : ?>
									<li class="att_on"><a href="<?=($this->session->userdata('is_paid')===TRUE || $open_ticker===TRUE)? '/'.KW.'_stock/recipe/growth':'javascript:;'?>">이익성장주</a></li>
									<?php $at_count++; endif; ?>

									<?php if($mri_data['m_biz_moat_score'] >= 16) : ?>
									<li class="att_on"><a href="<?=($this->session->userdata('is_paid')===TRUE || $open_ticker===TRUE)? '/'.KW.'_stock/recipe/moat':'javascript:;'?>">소비자독점주</a></li>
									<?php $at_count++; endif; ?>
					...
					<?php if($this->session->userdata('is_paid')===TRUE || $open_ticker===TRUE) :?>
					<a href="/<?=KW?>_search/competitor/<?=$ticker_code?>" class="box">
					<?php else :?>
					<a href="javascript:<?=($this->session->userdata('is_newlink')===TRUE) ? 'fnSinChungNew();':'fnSinChung();'?>" class="box">
					<?php endif;?>
