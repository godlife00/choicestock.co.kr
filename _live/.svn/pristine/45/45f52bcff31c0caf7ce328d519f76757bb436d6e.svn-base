...
<?php endif;?>
            ...
            <div class="search_top">
                <div class="data_area">
                    <h2 class="title"><a href="/<?=HT?>_search/summary/<?=$ticker['tkr_ticker']?>"><?=($sec_ticker != '' && $pri_ticker != '') ? $pri_ticker_name:$ticker['tkr_name']?></a></h2>
                    <ul class="info">
...
<?php if($sec_ticker=='' && $pri_ticker=='') :?>
            <div class="sub_mid tabs_area">
                ...
                <ul class="tabs_5<?=($this->session->userdata('is_paid')===TRUE || $open_ticker===TRUE)? '':' prm_tabs'?>">
                    ...
                    ?>
						...
						<?php else :?>
						<li class="<?=$tab_active?>"><a href="/<?=HT?>_search/<?=$seg?>/<?=$ticker_code?>"><?=$tab_tit?></a></li>
						<?php endif;?>					
					<?php endforeach; ?>
                </ul>
                <!-- //tabs_5 -->
        ...
        <?php if($current_tab == 'invest_charm') : // 종목진단은 내부 텝 3개 공통 상단을 여기서 그려줌 ?>
				...
				<ul class="tabs_3 tabs_depth2<?=($this->session->userdata('is_paid')===TRUE) ? '':' prm_tabs'?>">
                    ...
                    <li class="<?=$current_subtab=='fairvalue' ? 'active' : ''?>"><a href="/<?=HT?>_search/fairvalue/<?=$ticker_code?>">적정주가</a></li>
				</ul>
				<?php endif;?>

		<?php elseif($current_tab == 'financials') :?>
                <ul class="tabs_3 tabs_depth2">
                    <li class="<?=$current_subtab=='financials' ? 'active' : ''?>"><a href="/<?=HT?>_search/financials/<?=$ticker_code?>">재무제표</a></li>
					<li class="<?=$current_subtab=='finance_chart' ? 'active' : ''?>"><a href="/<?=HT?>_search/finance_chart/<?=$ticker_code?>">재무차트</a></li>
                    <li class="<?=$current_subtab=='invest' ? 'active' : ''?>"><a href="/<?=HT?>_search/invest/<?=$ticker_code?>">투자지표</a></li>
                </ul>
