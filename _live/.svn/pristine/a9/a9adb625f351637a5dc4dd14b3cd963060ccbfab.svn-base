            ...
            <div class="search_top">
                <div class="data_area">
                    <?php if($is_soft === true) :?>
                    <h2 class="title"><a href="/<?=KW?>_search/invest_charm/<?=$ticker['tkr_ticker']?>"><?=($sec_ticker != '' && $pri_ticker != '') ? $pri_ticker_name:$ticker['tkr_name']?></a></h2>
                    <?php else :?>
                    <h2 class="title"><i class="attention<?=($myitem['mi_like']=='Y') ? ' on':''?>" id="catch_icon_one" onClick="javascript:fnMyitem('<?=$ticker['tkr_ticker']?>', '<?=urlencode(str_replace('/index.php/', '', $_SERVER["PHP_SELF"]));?>', 'Y', 'one');"></i><a href="/<?=KW?>_search/invest_charm/<?=$ticker['tkr_ticker']?>"><?=($sec_ticker != '' && $pri_ticker != '') ? $pri_ticker_name:$ticker['tkr_name']?></a></h2>
                    <?php endif;?>                    
...
<?php //endif;?>
                ...
                <ul class="info_icon">
                    ...
                    <li><?php if($this->session->userdata('is_paid')===TRUE) :?><a href="/<?=KW?>_stock/recipe/dividend"><?php endif;?>배당매력주<?php if($this->session->userdata('is_paid')===TRUE) :?></a><?php endif;?></li>
                    <?php endif; ?>

                    <?php if($mri_data['m_biz_growth_score'] >= 16) : ?>
                    <li><?php if($this->session->userdata('is_paid')===TRUE) :?><a href="/<?=KW?>_stock/recipe/growth"><?php endif;?>이익성장<?php if($this->session->userdata('is_paid')===TRUE) :?></a><?php endif;?></li>
                    <?php endif; ?>

                    <?php if($mri_data['m_biz_moat_score'] >= 16) : ?>
                    <li><?php if($this->session->userdata('is_paid')===TRUE) :?><a href="/<?=KW?>_stock/recipe/moat"><?php endif;?>소비자독점주<?php if($this->session->userdata('is_paid')===TRUE) :?></a><?php endif;?></li>
                    <?php endif; ?>
...
<?php if($sec_ticker=='' && $pri_ticker=='') :?>
            <div class="sub_mid tabs_area">
                <ul class="tabs_5<?=($this->session->userdata('is_paid')===TRUE || $open_ticker===TRUE)? '':' prm_tabs'?>">
                    ...
                    ?>
						...
						<?php else :?>
						<li class="<?=$tab_active?>"><a href="/<?=KW?>_search/<?=($seg=='summary')? 'invest_charm' : $seg?>/<?=$ticker_code?>"><?=$tab_tit?></a></li>
						<?php endif;?>					
					<?php endforeach; ?>
                </ul>
                <!-- //tabs_5 -->
        ...
        <?php if($current_tab == 'summary') : // 종목진단은 내부 텝 3개 공통 상단을 여기서 그려줌 ?>
                <ul class="tabs_3 tabs_depth2">
                    <li class="<?=$current_subtab=='invest_charm' ? 'active' : ''?>"><a href="/<?=KW?>_search/invest_charm/<?=$ticker_code?>">투자매력</a></li>
                    <li class="<?=$current_subtab=='summary' ? 'active' : ''?>"><a href="/<?=KW?>_search/summary/<?=$ticker_code?>">기업개요</a></li>
                    <li class="<?=$current_subtab=='finance_chart' ? 'active' : ''?>"><a href="/<?=KW?>_search/finance_chart/<?=$ticker_code?>">재무차트</a></li>
                </ul>
