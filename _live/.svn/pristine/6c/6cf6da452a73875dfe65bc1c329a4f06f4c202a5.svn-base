...
<?php if($sec_ticker=='' && $pri_ticker=='') {?>
            <div class="search_financials_area">
                <div class="tabs_menu">
                    <span onclick="location.href='/<?=KW?>_search/financials/<?=$ticker_code?>/MRT';" class="<?=$dimension=='MRT' ? 'active' : ''?>">연환산</span>
                    <span onclick="location.href='/<?=KW?>_search/financials/<?=$ticker_code?>/MRY';" class="<?=$dimension=='MRY' ? 'active' : ''?>">연간</span>
                    <span onclick="location.href='/<?=KW?>_search/financials/<?=$ticker_code?>/MRQ';" class="<?=$dimension=='MRQ' ? 'active' : ''?>">분기</span>
                </div>
                <!-- //tabs_menu -->
...
<?php }else{?>
        ...
        <div class="sub_mid nondata">
            <p class="nodata_guide"><strong><?=$sec_ticker;?></strong>의 상세 기업정보는 <strong><a href="/<?=KW?>_search/summary/<?=$pri_ticker?>"><?=$pri_ticker?></a></strong> 종목에서 확인할 수 있습니다.</p>
        </div>
        <!-- //sub_mid nondata -->
<?php }?>