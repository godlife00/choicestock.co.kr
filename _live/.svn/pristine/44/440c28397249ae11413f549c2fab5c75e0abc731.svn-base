...
//print_r($data['mrt_list']);
    ...
    public function recipe($type='dividend') {
        ...
        $data['score_content_html'] = $this->load->view('/mobile/stock/recipe_list.php', array('type' => $type, 'list' => $list), true);
		$data['type'] = $type;
    ...
    public function vod_view($no=0) {
        ...
        if($row['vm_is_active'] != 'YES' || $row['vm_display_date'] > date('Y-m-d H:i:s') || ($row['vm_view_srv'] != '' && $row['vm_view_srv'] != 'C')) {
            $this->common->historyback();
            return;
        }
    ...
    public function research_view($no=0) {
        ...
        if($row['e_is_active'] != 'YES' || $row['e_display_date'] > date('Y-m-d H:i:s') || ($row['e_view_srv'] != '' && $row['e_view_srv'] != 'C')) {
            $this->common->historyback();
            return;
        }
				...
				foreach($ticker_list as $key => $ticker) {
					...
					if($ticker['tkr_ticker']!='') {
						...
						if($choice_ticker != '' && $choice_name != '') {
							$industry_commnet = '<span class="same_selt"><a href="/search/invest_charm/'.$choice_ticker.'">'.$choice_name.'</a>와(과) 같은 업종입니다</span>';
						}

$catch_list = <<<PHPSKIN
<div class="chart_catch">
	<div class="data_area">
		<h2 class="title"><a href="/search/invest_charm/{$ticker['tkr_ticker']}#">{$ticker_info[$ticker['tkr_ticker']]['tkr_name']}</a></h2>
		<ul class="info">
				...
				foreach($ticker_list as $key => $ticker) {
					...
					if($ticker['tkr_ticker']!='') {
						...
						if(sizeof($analysis_data) > 0) {
							...
							$ticker_info[$ticker['tkr_ticker']]['analysis_link'] = '/stock/analysis_view/'.$analy['an_id'];
						}
						*/
