            <div class="sub_top">
                <div class="">
                    <div class="txt_box">
                        <p class="txt">전일 미국증시 주요지수와 포트폴리오 주요소식, 시황을 매일 아침 미주미초이스 프리미엄 회원들께 발송합니다.</p>
                    </div>
                    <!-- //txt_box -->
                </div>
            </div>
                ...
                <ul id="analysis_list" class="lst_type">
                    ...
                    <li>
                        <dl class="lst_type2 none_img<?=($this->session->userdata('is_paid')===FALSE) ? ' lst_lock':''?>">
                            ...
                            <?php else :?>
                            <a href="/<?=KW?>_stock/morning_view/<?=$exp['mo_id']?>">
                            <?php endif;?>
            ...
            function view_more() {
                ...
                page += 1;
                $.post('/<?=KW?>_stock/ajax_get_morning_list', {'page': page}, function(res) {
                    if($.trim(res).length) {
