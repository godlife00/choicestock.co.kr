...
class Wt_main extends Wt_BaseMobile_Controller {
	...
	public function iosopen()
	{
		$return_url = $this->input->get('return_url');

		if($return_url != '' && $this->session->userdata('is_login') && $this->session->userdata('user_key') != '' && $this->session->userdata('user_phone') != '') {
			...
			$this->load->view('/'.WT.'/main/iosopen', $data);
		}
	}

	public function adropen()
	{
		$return_url = $this->input->get('return_url');
		...
		if($return_url != '' && $this->session->userdata('is_login') && $this->session->userdata('user_key') != '' && $this->session->userdata('user_phone') != '') {
			...
			$data = array();
			$data['return_url'] = $return_url;
			$this->load->view('/'.WT.'/main/appopen', $data);
		}
	}
	...
	*/
    ...
    public function getWtMarketPrice($tickers) 
	{
		if($tickers != '') {
			$result = $this->common->get_wtsise($tickers);
			exit($result);
		}
	}
	...
	{
		//메인지수
		$result = $this->common->get_wtsise('DJI-DJI|NAS-IXIC|CME-NQ|CME-ES');
		exit($result);
	}
    ...
    public function index()
    {
		...
		if( is_file($file_path) ) {
			$soaring = json_decode(file_get_contents($file_path), true);
			$data['soaring'] = array_slice($soaring['one'], 0, 3);
			unset($soaring);
		}
		...
		if( is_file($file_path) ) {
			...
			$data['interest'] = array_slice($interest, 0, 3);
			unset($interest);
		}
		...
		if($this->session->userdata('is_login') === true) {
			...
			$myitem_extra = array(
				'fields' => 'tkr_ticker, tkr_name, tkr_close, tkr_rate ',
				'slavedb' => true,
				'order_by' => 'tkr_rate DESC',
				'limit' => '3',
			);

			$data['myitem'] = $this->myitem_tb_model->getList($myitem_params, $myitem_extra)->getData();
		}
		else {
			$data['myitem'] = '';
		}
		...
		if( is_file($file_path) ) {
            $recommend_all = json_decode(file_get_contents($file_path), true);
		...
		//한경TV동영상
		$vod_file = 'wowtv_vod.json';
		//$file_path = WEBDATA.'/json/'.$vod_file;
        $file_path = str_replace('hoon','datahero',WEBDATA).'/json/'.$vod_file;

		if( is_file($file_path) ) {
             $data['wowtv_vod_list'] = array_slice(json_decode(file_get_contents($file_path), true), 0, 2);
		}

		//모닝브리핑 배너(2021.4.13추가)
		$data['bn1_txt'] = '';
		$data['bn1_id'] = '';
		...
		if( is_file($file_path) ) {
			...
			foreach($morning_list as $morning) {
				if($morning['e_display_date'] <= date('Y-m-d H:i:s')) {
					$data['bn1_txt'] = iconv_substr($morning['e_title'], 0, 40, 'utf-8');
					$data['bn1_id'] = $morning['e_id'];
					break;
				}
			}
		}
		...
		$data['news_list'] = $response['data']['list'];
		...
		if( is_file($file_path) ) {
            $note_all = json_decode(file_get_contents($file_path), true);

			$cnt=0; 
			$note_list = array();
			foreach($note_all as $note) {
				if($note['e_display_date'] > date('Y-m-d H:i:s') || $note['e_view_wt'] != 'Y') continue;
				if($cnt>2) break;
				$note_list[] = $note;
				$cnt++;
			}	
		...
		//메인지수
		$wtsise = $this->common->get_wtsise('DJI-DJI|NAS-IXIC|CME-NQ|CME-ES');
		$data['wtsise_list'] = json_decode($wtsise, true);
		$data['wtsise_namelist'] = array('다우 산업','나스닥 종합','나스닥 선물','S&P 선물');

        $this->header_data['header_template'] = '1';
        $this->header_data['head_url'] = '/';
        $this->header_data['show_alarm'] = TRUE;

        $data['meta_title'] = '미국주식투자자를 위한 서비스 - 미국주식창';
...
//echo '<pre>'; print_r($tab_stock_data);
    ...
    public function menu() {
        $this->header_data['header_template'] = '';
    ...
    public function saveTicker() {
        ...
        $this->common->locationhref('/'.WT.'_main/search');
        exit;
    }
...
*/
	...
	public function service_info() {
        $this->load->view('/'.WT.'/main/service_info');
    }

	public function bridge($type='A') {

		$seq = $this->input->get('seq');
		if($seq == '') $seq = $this->input->post('seq');

		$data = array();
		$data['type'] = $type;
		$data['seq'] = $seq;
        $this->load->view('/'.WT.'/main/movetoapp', $data);
	}
}
