...
<?php endif;?>			
            <div class="main_top recommend_area">
                <h3 class="title"><a href="/<?=KW?>_stock/recommend">종목추천</a></h3>
						...
						?>
                            <div class="swiper-slide">
                                <div class="area">
                                    <ul class="list">
										<?php if($this->session->userdata('is_paid')===TRUE) :?>
                                        <li class="title"><a href="/<?=KW?>_stock/recommend_view/<?=$val['rc_id']?>"><?=$val['ticker']['tkr_name']?></a></li>
                                        <li class="sum"><?=$val['ticker']['tkr_ticker']?></li>
										<?php else:?>
                                        <li class="title"><a href="javascript:<?=($this->session->userdata('is_newlink')===TRUE) ? 'fnSinChungNew();':'fnSinChung();'?>"><?=iconv_substr($val['ticker']['tkr_name'], 0, 1, 'utf-8')?><span class="remark"><div class="txt_filter size_M"><i></i><i></i><i></i><i></i></div></span></a></li>
                                        ...
                                        <li class="num">
                                            <div>
												...
												<?php else :?>
                                                <span class="td prm_lock"><a href="javascript:<?=($this->session->userdata('is_newlink')===TRUE) ? 'fnSinChungNew();':'fnSinChung();'?>"><img src="/img/prm_lockW.png" alt="잠김"></a></span>
												<?php endif;?>
                                            </div>
                                            <div>
