...
<?php 
	$is_wowinfo = false;
	if($is_open === true) {
		//$wval = $this->common->get_wowtv_info($rc_data['rc_ticker']);
		$wval = array_shift(json_decode($this->common->get_wtsise($rc_data['rc_ticker']), true));
		if(is_array($wval) && sizeof($wval)) {
			$is_wowinfo = true;
		}
	}
?>
<?php if($is_wowinfo == true &&  $wval['status'] == '0'):?>
<script type="text/javascript">
<!--
var pre_price = <?=$rc_data['ticker']['tkr_close']?>;
function ajax_wtsise() {
	$.ajax({
        url: '/wt_main/getWtMarketPrice/<?=$rc_data['rc_ticker']?>',
		type : 'post',
		dataType : 'json',
		async: false,
		cache : false,
...
<?php endif;?>
            <div class="sub_top view">
                <div class="chart_area">
                    <div class="chartData">
                        <h2 class="title" title="<?=$rc_data['ticker']['tkr_name_en']?>"><a href="/<?=WT?>_search/summary/<?=$rc_data['rc_ticker']?>"><?=$rc_data['ticker']['tkr_name']?></a></h2>
                        <a href="/<?=WT?>_search/summary/<?=$rc_data['rc_ticker']?>" class="more"><img src="/img/more_Black.png" alt="더보기"></a>
                        <ul class="info">
                            <li class="sum"><span class="eng"><?=$rc_data['rc_ticker']?></span> </li>
							<?php if($rc_data['rc_portfolio'] == 'Y') :?><li class="port">포트편입</li><?php endif;?> 
                        </ul>
                        <!-- //info -->
                        <ul class="detail">
							...
							<li class="per">
								<span class="<?=$wval['diff_rate'] > 0 ? 'increase' : 'decrease'?>" id='diff'><?=($wval['diff_rate']>0) ? '+ ':''?><?=$wval['diff_price']?> <span>(<?=($wval['diff_rate']>0) ? '+':''?><?=$this->common->set_pricepoint($wval['diff_rate'], '2')?><b>%</b>)</span></span>
							</li>
							<li class="day" id='price_date'><?=($wval['status'] == '0') ? date('y.m/d H:i', strtotime($wval['updated_at'])) : date('y.m/d', strtotime($wval['updated_at']))?>, USD<?=($wval['status'] == '0') ? ' (​15분 지연시세)':''?></li>
							<?php else :?>
                            <li class="num" id='last_price'><?=$this->common->set_pricepoint(number_format($rc_data['ticker']['tkr_close'], 2), '1')?></li>
                            <li class="per">
                                <span class="<?=$class?>" id='diff'><?=$this->common->set_pricepoint($rc_data['ticker']['tkr_diff_str'], '1')?> <span>(<?=$this->common->set_pricepoint($rc_data['ticker']['tkr_rate_str'], '2')?>)</span></span>
                            </li>
                            <li class="day" id='price_date'><?php if(isset($rc_data['ticker']['tkr_lastpricedate'])&&$rc_data['ticker']['tkr_lastpricedate']) echo date('y.m/d', strtotime($rc_data['ticker']['tkr_lastpricedate'])).', ';?> USD</li>
							<?php endif;?>
                        </ul>
                        ...
                        <div class="detail_data">
                            <table cellspacing="0" border="1" class="tableRanking">
                                ...
                                <tbody>
                                    <tr>
                                        <th class="recom"><span>추천가<span class="day"><?=date('y.m/d', strtotime($rc_data['rc_recom_date']))?></span></span></th>
                            ...
                            <div class="attainment_box">
                                <span class="title"><i></i> 목표가달성</span>
                                <span class="percent"><i></i> <?=number_format((($rc_data['rc_goal_price']/$rc_data['rc_recom_price'])-1)*100,2)?><b>%</b></span>
                            </div>

							<?php elseif($rc_data['rc_endtype'] == 'FAIL') :?>
                            <div class="attainment_box">
                                <span class="title"><i></i> 손절가매도</span>
                                <span class="percent"><i></i> <?=number_format((($rc_data['rc_giveup_price']/$rc_data['rc_recom_price'])-1)*100,2)?><b>%</b></span>
                            </div>

							<?php elseif($rc_data['rc_endtype'] == 'SELL') :?>
                            <div class="attainment_box">
                                <span class="title"><i></i> 교체매도</span>
                                <span class="percent"><i></i> <?=number_format((($rc_data['rc_mid_price']/$rc_data['rc_recom_price'])-1)*100,2)?><b>%</b></span>
                            </div>
                ...
                <div class="tabsArea">
                    ...
                    <div class="tab_container">
                        ...
                        <div id="tab1" class="tab_content">
							...
							<div class="one_step">
                                <span class="txt">전체 포트폴리오와 수익률을 확인하세요.</span>
                                <a href="/<?=WT?>_stock/recommend/portfolio" class="more"><span>포트폴리오</span></a>
                            </div>
                            <div class="catalyst">
                                <h4 class="title">유의사항</h4>
                                <div>
                                    <p><i>①</i>종목 추천은 주 2회(화, 목) 진행합니다.</p>
                                    <p><i>②</i>목표가, 손절가 도달시 일괄 매도를 원칙으로 합니다.</p>
                                    <p><i>③</i>개인별 종목상담은 불가능하오니 목표가, 손절가를 꼭 지켜
                                        주시기 바랍니다.</p>
                                    <p><i>④</i>포트폴리오 최대 보유 종목수는 33종목입니다.(장기보유 9종목, 3개월 보유 24종목) 단, 목표가&#47;손절가 도달시 일괄 매도를 원칙으로 합니다.</p>
                                    <p><i>⑤</i>포트폴리오 종목별 투자비중은 3%입니다.</p>

                                    <div class="bandchart_more">
                                        <p><a href="/<?=WT?>_stock/recommend/portfolio#more"><i></i>종목추천&amp;포트폴리오 운영안내 >></a></p>          
                                    </div>
                                </div>
                            </div>
                        ...
                        <div id="tab2" class="tab_content">
                            ...
                            <a href="/<?=WT?>_search/invest_charm/<?=$rc_data['rc_ticker']?>" class="more"><img src="/img/more_Black.png" alt="더보기"></a>

                            <div id="spider_chart" class="containerSpider_1"></div>
                            <script>SubRecomSpiderChart('spider_chart', [<?=$rc_data['m_future_stars']?>,<?=$rc_data['m_biz_moat_stars']?>,<?=$rc_data['m_biz_cashflow_stars']?>,<?=$rc_data['m_biz_growth_stars']?>,<?=$rc_data['m_biz_safety_stars']?>])</script>

                            <!-- 투자매력도 id = containerSpider_1 -->

                            <span class="charm_num">
                                <strong><?=$rc_data['m_biz_total_score']?></strong> / 100
                                <span class="total">투자매력</span>
                            </span>
                            <!-- //charm_num -->
                            <?php endif; ?>
                            ...
                            <div class="data_table">
                                ...
                                <a href="/<?=WT?>_search/invest/<?=$rc_data['rc_ticker']?>" class="more"><img src="/img/more_Black.png" alt="더보기"></a>
                                <!-- 수익성(연환산) -->
                                <div>
                                    <table cellspacing="0" border="1" class="tableRanking">
                                        <colgroup>
