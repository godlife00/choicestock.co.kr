			<div class="main_mid note_area">
                <h3 class="title"><?php if($this->session->userdata('is_paid')===TRUE):?><a href="/<?=SK?>_stock/note">종목노트</a><?php else:?>종목노트<?php endif;?></h3>
				<?php if($this->session->userdata('is_paid')===TRUE) :?>
				<a href="/<?=SK?>_stock/note" class="more"><img src="/img/more_Black.png" alt="더보기"></a>                    
				<?php endif;?>
				<ul class="lst_type">
					...
					<li>
						<dl class="lst_type2<?=($note['e_pay']=='Y' && $this->session->userdata('is_paid')===FALSE) ? ' lst_lock':''?>">
							<dt class="tit">
								...
								<?php else :?>
								<a href="/<?=SK?>_stock/research_view/<?=$note['e_id']?>">
								<?php endif;?>
