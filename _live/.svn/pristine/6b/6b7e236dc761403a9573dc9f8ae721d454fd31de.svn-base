...
abstract class Hn_BaseMobile_Controller extends CI_Controller {
...
//https://hana.choicestock.co.kr/hn_main?serviceYN=Y&customNo=SUY4UUs3M1JKTmhSTFREL0NVa1F2dz09
    }

    private function _host_check() {
        $host = $_SERVER['HTTP_HOST'];
        if(strstr($host, 'hana2.choicestock.co.kr')) {
    ...
    public function member_check($user_name='') {
		if($user_name == '' || $this->session->userdata('is_paid')==FALSE) return;

		$this->load->model(DBNAME.'/member_tb_model');

		if($this->member_tb_model->get(array('m_name' => $user_name))->isSuccess()) {
			$member_info = $this->member_tb_model->getData();

			$user_id = $member_info['m_id'];
		}
		else {
			$user_id = uniqid('hn_');
			$params = array(
...
//서비스소개 : "https://hana.choicestock.co.kr/hn_main/service"  + &serviceYN="Y 또는 N" &customNo="고객번호"
//홈 :  "https://hana.choicestock.co.kr/hn_main" + &serviceYN="Y 또는 N" &customNo="고객번호"
//customNo : 고정64
    ...
    protected function payCheck(){
        if(!$this->session->userdata('is_paid')){
			echo '<script>';
			echo '    window.location = window.location = "callScreenMethod://OpenPopup?param=2313REG";';
			echo '</script>';
            $this->common->locationhref('/hn_main');
            exit;
		}
    }
...
/* remove
                ...
                case 'member/notice':
                    ...
                    break;
remove */
                case HN.'_main/myticker':
                    $show_menu = false;
                case HN.'_main/onestop':
                case HN.'_main/search':
                    $container_class = 'sub_search schfix_inc';
                    $show_type = '2';
                    $is_searching = true;
                    $searching_title = '진단';
                    break;
                case HN.'_stock/recommend':
                    $header['header_class'] = ' h_sub';
                ...
                case HN.'_stock/recommend_view':
                    ...
                    $is_sicon = false;
                    break;
                case HN.'_stock/analysis':
                    $header['header_class'] = ' h_sub';
                    $container_class = 'sub_analysis';
                    $show_type = '2';
                    //$is_vt_banner = TRUE;
                    break;
                case HN.'_stock/analysis_view':
                    $container_class = 'sub_analysis';
                    $show_type = '2';
                    break;
                case HN.'_stock/winner':
                    $header['header_class'] = ' h_sub';
                ...
                case HN.'_stock/vod_view':
                    ...
                    $soft_menu = 'attractive';
                    break;
                case HN.'_search/news_view':
                case HN.'_stock/research_view':
                    $container_class = 'sub_research';
                    $show_type = '2';
                    $is_headtop = true;
                    $headtop_title = '탐구생활';
                    ...
                    $soft_menu = 'attractive';
                    break;
                case HN.'_stock/morning_view':
                case HN.'_stock/morning':
                ...
                case HN.'_stock/catch_info':
                    ...
                    $searching_title = '발굴';
                    break;
                case HN.'_search/primary_ticker':
                case HN.'_search/news':
                case HN.'_search/fairvalue':
                case HN.'_search/invest_charm':
                case HN.'_search/summary':
                case HN.'_search/finance_chart':
                case HN.'_search/invest':
                case HN.'_search/financials':
                case HN.'_search/dividend':
                case HN.'_search/competitor':
                    $show_type = '2';
                    ...
                    $soft_menu = 'recommend';
                    break;
                case HN.'_attractiveness/attractive':
                    $header['header_class'] = ' h_sub';
                    $show_type = '2';
                ...
                case HN.'_main/menu':
                    ...
                    break;
                case HN.'_main/service':
                case HN.'_main/service_guide':
                case HN.'_main/service_prm':
                    $show_type = '3';
                    //$is_vt_banner = TRUE;
...
//remove        case 'payment/pay_free':
		...
		*/
		...
		$header['part_page'] = $this->partner_info['part_page'];
		$footer['part_page'] = $this->partner_info['part_page'];

        if( ! $return_contents) {

            $this->load->view('/'.HN.'/inc/header.php', $header);
            $this->load->view('/'.HN.'/'.$view.'.php', $data);
            $this->load->view('/'.HN.'/inc/footer.php', $footer);
        } else {

            //if($data['chkUri']!='hana') {
            $result  = $this->load->view('/'.HN.'/inc/header.php', $header, true)."\n";
            $result .= $this->load->view('/'.HN.'/'.$view.'.php', $data, true)."\n";
            //}
            $result .= $this->load->view('/'.HN.'/inc/footer.php', $footer, true)."\n";
            return $result;
        }
    }
