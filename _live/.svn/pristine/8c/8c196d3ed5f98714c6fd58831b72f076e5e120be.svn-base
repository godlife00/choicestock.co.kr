function fnLogin() {
	window.android.requestLogin();
}

function fnPay() {
	window.android.requestPayment();
}
	...
	$.ajax({
		...
		success : function(data) {
			if(data.success) {
				...
				location.href='/px_stock/catch_info?catch_list='+data.catch_list+'&sector_list='+data.sector_list;
			}
			else {
				alert(data.msg);
				location.href='/px_stock/catch_info';
			}
		}
	});
}
...
function fnMyTickerSave() {
	...
	location.href= '/px_main/saveTicker?ticker='+ticker;
}
...
function fnMyitem(ticker, page, count, id) {
	...
	var href = '/px_search/setTicker/'+ticker+'/'+count;
...
function moveToSearch(div) {
    ...
    if ($('li a._on', div).length > 0) {
        ...
        location.href = '/px_search/invest_charm/' + keyword;
    } else {
...
function onSearchTicker(ticker) {
	...
	location.href = '/px_search/invest_charm/' + ticker;
}
...
function setSearchHistory(keyword) {
    ...
    */
    ...
    $.ajax({
        url: '/px_main/ajax_save_search_history',
        type: 'GET',
        data: { 'ticker': keyword },
        async: false,
        success: function () {
