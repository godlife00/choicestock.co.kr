function fnSinChungList() {
    var formid = $("#paylist_form");
    formid.submit();
}
...
function fnNoticeSet(obj) {
	...
	href = '/wt_member/notice_setup/'+obj.name+'/'+val;
	...
	$.ajax({
		...
		success : function(data) {
			if (data.error) {
				HK.showToast(data.error);
				if(data.res == '1') {
					location.href='/wt_main';
				}
				return;
			}
			else if (data.success) {
				HK.showToast(data.res);
				return;
			}
		}
	});
    ...
    $.ajax({
        ...
        success : function(data) {
            if(data.success) {
                ...
                location.href='/wt_stock/catch_info?catch_list='+data.catch_list+'&sector_list='+data.sector_list;
            }
            else {
                //alert(data.msg);
				HK.showToast(data.msg);
                location.href='/wt_stock/catch_info';
            }
        }
    });
}
...
function fnCatchNewSave() {
	...
	$('#loading').css("display", "block");
	location.href='/wt_stock/catch_process?catchType='+catchType+'&catchSize='+catchSize+'&catchSector='+catchSec;
}
...
function fnNextStep(no) {
    ...
    if(no=='1') {
        if( $('#catchType').children().hasClass('active') == false ) {
			...
			HK.showToast('선호하는 종목의 유형을 선택해주세요.');
            return;
        }
        else {
            $('.setting_pop').removeClass('open');
        ...
        if( $('#catchSize').children().hasClass('active') == false ) {
			...
			HK.showToast('선호하는 종목의 규모를 선택해주세요.');
            return;
        }
        else{
            $('.setting_pop').removeClass('open');
...
function fnMyTickerSave() {
    ...
    location.href= '/wt_main/saveTicker?ticker='+ticker;
}
...
function fnMyitem(ticker, page, count, id) {
    ...
    if (ticker == '') {
        //alert('선택한 종목이 없습니다.');
		HK.showToast('선택한 종목이 없습니다.');
        return;
    }

    var href = '/wt_search/setTicker/'+ticker+'/'+count;
...
function moveToSearch(div) {
    ...
    if ($('li a._on', div).length > 0) {
        ...
        location.href = '/wt_search/summary/' + keyword;
    } else {
...
function onSearchTicker(ticker) {
    ...
    location.href = '/wt_search/summary/' + ticker;
}
...
function setSearchHistory(keyword) {
    ...
    */
    ...
    $.ajax({
        url: '/wt_main/ajax_save_search_history',
        type: 'GET',
        data: { 'ticker': keyword },
        async: false,
        success: function () {
