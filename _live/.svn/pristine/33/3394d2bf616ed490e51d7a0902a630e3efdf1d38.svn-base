    ...
    $.ajax({
        ...
        success : function(data) {
            if(data.success) {
                ...
                location.href='/sk_stock/catch_info?catch_list='+data.catch_list+'&sector_list='+data.sector_list;
            }
            else {
                alert(data.msg);
                location.href='/sk_stock/catch_info';
            }
        }
    });
}
...
function fnCatchNewSave() {
	...
	$('#loading').css("display", "block");
	location.href='/sk_stock/catch_process?catchType='+catchType+'&catchSize='+catchSize+'&catchSector='+catchSec;
}
...
function fnMyTickerSave() {
    ...
    location.href= '/sk_main/saveTicker?ticker='+ticker;
}
...
function fnMyitem(ticker, page, count, id) {
    ...
    var href = '/sk_search/setTicker/'+ticker+'/'+count;
...
function moveToSearch(div) {
    ...
    if ($('li a._on', div).length > 0) {
        ...
        location.href = '/sk_search/summary/' + keyword;
    } else {
...
function onSearchTicker(ticker) {
    ...
    location.href = '/sk_search/summary/' + ticker;
}
...
function setSearchHistory(keyword) {
    ...
    */
    ...
    $.ajax({
        url: '/sk_main/ajax_save_search_history',
        type: 'GET',
        data: { 'ticker': keyword },
        async: false,
        success: function () {
