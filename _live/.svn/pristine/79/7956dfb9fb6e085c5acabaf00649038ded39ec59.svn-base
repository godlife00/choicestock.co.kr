//신청
function fnSinChung() {
	window.location = "callScreenMethod://OpenPopup?param=2313REG";
}
//해지
function fnCancel() {
	window.location = "callScreenMethod://OpenPopup?param=2313UNREG";
}
//알림정보
function fnNotiInfo() {
	//alert('알림정보페이지 이동');
...
function fnCatchNewSave() {
	...
	$('#loading').css("display", "block");
	location.href='/hn_stock/catch_process?catchType='+catchType+'&catchSize='+catchSize+'&catchSector='+catchSec;
}
...
function moveToSearch(div) {
    ...
    if ($('li a._on', div).length > 0) {
        ...
        location.href = '/hn_search/summary/' + keyword;
    } else {
...
function onSearchTicker(ticker) {
    ...
    location.href = '/hn_search/summary/' + ticker;
}
