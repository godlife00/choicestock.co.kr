...
//print_r($data['mrt_list']);
    ...
    public function analysis() {
        ...
        $data['content_html'] = $this->load->view('/'.KW.'/stock/analysis_list', $list_data, true);
        $this->header_data['header_template'] = '2';
    ...
    public function ajax_get_analysis_list() {
        ...
        echo $this->load->view('/'.KW.'/stock/analysis_list', $list_data, true);
    ...
    public function recipe($type='dividend') {
        ...
        $data['score_content_html'] = $this->load->view('/'.KW.'/stock/recipe_list.php', array('type' => $type, 'list' => $list), true);
        $data['type'] = $type;
    ...
    public function ajax_get_recipe_list() {
        ...
        $content_html = $this->load->view('/'.KW.'/stock/recipe_list.php', array('type' => $type, 'list' => $list), true);
        echo $content_html;
        return;
    }
    ...
    public function vod_view($no=0) {
        ...
        if($row['vm_is_active'] != 'YES' || $row['vm_display_date'] > date('Y-m-d H:i:s') || ($row['vm_view_srv'] != '' && $row['vm_view_srv'] != 'M')) {
            $this->common->historyback();
            return;
        }
/*
...
*/
    ...
    public function research_view($no=0) {
        ...
        if($row['e_is_active'] != 'YES' || $row['e_display_date'] > date('Y-m-d H:i:s') || ($row['e_view_srv'] != '' && $row['e_view_srv'] != 'C') || $row['e_is_inside'] == 'Y' || $row['e_kiwoom_off'] == 'Y') {
            $this->common->historyback();
            return;
        }
    ...
    public function push_control($push='') {
        ...
        if( in_array($push, array('Y', 'N'))) {
            ...
            $this->common->locationhref('/'.KW.'_stock/view_close');
            exit;
        }
        else {
            $this->common->locationhref('/');
                ...
                foreach($ticker_list as $key => $ticker) {
                    ...
                    if($ticker['tkr_ticker']!='') {
                        ...
                        if($choice_ticker != '' && $choice_name != '') {
                            $industry_commnet = '<span class="same_selt"><a href="/'.KW.'_search/invest_charm/'.$choice_ticker.'">'.$choice_name.'</a>와(과) 같은 업종입니다</span>';
                        }
$pre = KW;
$catch_list = <<<PHPSKIN
                ...
                foreach($ticker_list as $key => $ticker) {
                    ...
                    if($ticker['tkr_ticker']!='') {
                        ...
                        if(sizeof($analysis_data) > 0) {
                            ...
                            $ticker_info[$ticker['tkr_ticker']]['analysis_link'] = '/'.KW.'_stock/analysis_view/'.$analy['an_id'];
                        }
                        */
