            ...
            <div class="sub_mid research_board">
                <div class="video_tabs">
                    <a href="/<?=EG?>_stock/research" class="active">종목톡톡</a>                    
                </div>

                <div class="set">
                    <span onclick="location.href='/<?=EG?>_stock/research/display_date';" class="<?=$order_by == 'display_date' ? 'active' : ''?>">최신글</span>
                    <span onclick="location.href='/<?=EG?>_stock/research/view_count';" class="<?=$order_by == 'view_count' ? 'active' : ''?>">인기글</span>
                </div>
            ...
            function view_more() {
                ...
                page += 1;
                $.post('/<?=EG?>_stock/ajax_get_research_list', {'page': page, 'order_by':'<?=$order_by?>'}, function(res) {
                    if($.trim(res).length) {
