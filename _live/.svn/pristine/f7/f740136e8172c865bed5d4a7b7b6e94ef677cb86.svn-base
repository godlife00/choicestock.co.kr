            ...
            <?php if($this->session->userdata('is_paid')===TRUE) :?>
                ...
                <div class="chart_catch">
                    <div class="data_area">
                        <h2 class="title"><a href="/<?=EG?>_search/invest_charm/<?=$ticker['tkr_ticker']?>"><?=$ticker['tkr_name']?></a></h2>
                        <ul class="info">
                    ...
                    <span class="same_selt"><a href="/<?=EG?>_search/invest_charm/<?=$choice_ticker?>"><?=$choice_name?></a>와(과) 같은 업종입니다</span>
                    <?php endif;?>
                </div>
                <!-- //chart_catch -->
                <?php $idx++; endforeach;?>
                    <?php if($is_next === true) :?>
                    <div class="next"><a href="/<?=EG?>_stock/add_catch?mode=auto&idx=<?=$idx?>&page=1" class="nextPage">다음 페이지</a></div>
                    <?php endif;?>
                <?php else :?>
