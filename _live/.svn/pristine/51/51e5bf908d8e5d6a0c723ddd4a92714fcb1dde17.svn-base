                ...
                <div class="alarmList">
                    <ul>
						<?php foreach($header_data['noti_list'] as $noti_item) :
							$noti_item['nt_url'] = str_replace('/stock/','/'.X1.'_stock/',$noti_item['nt_url']);
							$noti_item['nt_url'] = str_replace('/search/','/'.X1.'_search/',$noti_item['nt_url']);
						?>
						<li>
							<a href="<?=(strlen($noti_item['nt_url']) > 3 ) ? $noti_item['nt_url'] : '#'?>">
								...
								<?php if($noti_item['nt_table']=='recommend_tb') :?>
									...
									<?php else :?>
										<?=$noti_item['nt_title']?><?=($noti_item['nt_ticker_name'] !='' && $noti_item['nt_ticker'] !='') ? ' '.iconv_substr($noti_item['nt_ticker_name'], 0, 1, 'utf-8').'<span class="remark"><div class="txt_filter size_S"><i></i><i></i><i></i><i></i></div></span>':''?>
									<?php endif;?>
								<?php else :?>
								<?=$noti_item['nt_title']?>
								<?php endif;?>
