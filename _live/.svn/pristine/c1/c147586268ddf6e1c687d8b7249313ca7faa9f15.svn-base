...
require_once dirname(__FILE__).'/x1_base_mobile.php';
class X1_search extends X1_BaseMobile_Controller {
    ...
    public function index() {
        $this->common->locationhref('/'.X1.'_main');
    }
    ...
    private function _general_data($ticker, $dimension='MRY', $option=array()) {
        ...
        if(sizeof($recom_data) > 0) {
            ...
            $data['recom_link'] = '/'.X1.'_stock/recommend_view/'.$recom['rc_id'];
        }
/*
        // 종목추천에 등록된 ticker 인지 확인
...
*/
		...
		//15min
		//$data['is_open'] = $this->is_open;
        $data['ticker_header'] = $this->load->view('/'.X1.'/search/ticker_header', $data, true);
...
*/
	public function setTicker($ticker='', $count='') {
        ...
        if( $this->session->userdata('is_login') === true ) {
            ...
            $result = array('error' => '관심종목은 유료 서비스 입니다.', 'res' => '1');
            exit(json_encode($result));
        }
	}
