                ...
                ?>
                    <div class="chart_area">
                        <div class="chartData">
                            ...
                            <ul class="info">
                                <li class="sum"><span class="eng"><?=($this->session->userdata('is_paid')===FALSE) ? '<span class="remark"><div class="txt_filter size_M"><i></i><i></i><i></i><i></i></div></span>':$val['ticker']['tkr_ticker']?></span> </li>
                            </ul>
                            <ul class="detail">
                                <?php /*if($is_open === true) :
								...
								php else :*/?>
                                <li class="num"><span><?=$this->common->set_pricepoint(number_format($val['ticker']['tkr_close'], 2), '1')?></span></li>
                    ...
                    <?php if($this->session->userdata('is_paid')===TRUE) :?>
						<div class="swiper-slide">
                            <div class="attention_all">
                                <a href="/<?=X1?>_main/search">관심종목 전체보기 <i></i></a>
                            </div>                            
                        </div>
                        <!-- //swiper-slide -->    
                    <?php endif;?>
...
<script>
	$(document).ready(function(){
	<?php for($i=0; $i<sizeof($catch_ticker_data); $i++) :?>
		  $('#cc_price<?=$i?>').animate({'background-color':'#dddddd'}, 500).animate({'background-color':'transparent'}, 500);
		  $('#cc_diff<?=$i?>').animate({'background-color':'#dddddd'}, 500).animate({'background-color':'transparent'}, 500);
	<?php endfor;?>
	});
</script>			
<?php endif;?>
				<div class="list_area d01">
                    <div class="swiper-container catchSwiper">
                        <div class="swiper-wrapper">
                            <?php $cc_cnt=0; foreach($catch_ticker_data as $key => $val) : ?>
                            <?php
                                $class = 'decrease';
                                if($val['ticker']['tkr_rate'] > 0) {
                                    $class = 'increase';
                            ...
                            <div class="swiper-slide">
                                <ul class="list">
									...
									<?php if($is_wowinfo === true) :?>
                                    <li class="num">
                                        <span id='cc_price<?=$cc_cnt?>'><?=$this->common->set_pricepoint(number_format($wval['last_price'], 2), '1')?></span>
                                        <span class="<?=($wval['diff_rate']>0) ? 'increase':'decrease'?>" id='cc_diff<?=$cc_cnt?>'><?=($wval['diff_rate']>0) ? '+':''?><?=$this->common->set_pricepoint($wval['diff_rate'], '2')?><b>%</b></span>
                                    </li>
									<?php else :?>                            
                                    <li class="num">
                                        <span><?=$this->common->set_pricepoint(number_format($val['ticker']['tkr_close'], 2), '1')?></span>
...
<script>
	$(document).ready(function(){
	<?php for($i=0; $i<sizeof($recent_report); $i++) :?>
		  $('#rr_price<?=$i?>').animate({'background-color':'#dddddd'}, 500).animate({'background-color':'transparent'}, 500);
		  //$('#rr_diff<?=$i?>').animate({'background-color':'#dddddd'}, 500).animate({'background-color':'transparent'}, 500);
	<?php endfor;?>
	});
</script>			
<?php endif;?>
                                ...
                                <table cellspacing="0" border="0" class="tableRanking type_2Line">
									...
									<?php if($rc>4) break;?>
                                        <tr>
											...
											<?php if($is_wowinfo === true) :?>
											<td class="num">
												<span id='rr_price<?=$rc?>'><?=$this->common->set_pricepoint(number_format($wval['last_price'], 2), '1')?></span>
												<span class="<?=($wval['diff_rate']>0) ? 'increase':'decrease'?>" id='rr_diff<?=$rc?>'><?=($wval['diff_rate']>0) ? '+':''?><?=$this->common->set_pricepoint($wval['diff_rate'], '2')?><b>%</b></span>
                                                <!-- increase 증가, decrease 감소 -->
                                            </td>
											<?php else :?>                            
											<td class="num">
                                ...
                                <table cellspacing="0" border="0" class="tableRanking type_2Line">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
											...
											$is_wowinfo = false;
											if($is_open === true) {
												$wval = $this->common->get_wowtv_info($row['m_ticker']);
												if(is_array($wval) && sizeof($wval)) {
													$is_wowinfo = true;
												}
											}
											...
											<?php else :?>                            
                                            <td class="num">
                                                <span><?=$this->common->set_pricepoint(number_format($row['tkr_close'], 2), '1')?></span>
								            ...
								            <?php if($this->session->userdata('is_paid')===FALSE) :?>
												<a href="javascript:fnLogin();">
                                                    <?=iconv_substr($row['m_korname'], 0, 1, 'utf-8')?><span class="remark">
                                                        <div class="txt_filter size_M"><i></i><i></i><i></i><i></i>
                                                        </div>
                                                    </span>
                                                    <span class="ticker">
                                                        <span class="remark">
                                                            <div class="txt_filter size_S"><i></i><i></i><i></i><i></i>
                                                            </div>
                                                        </span>
                                                    </span>
                                                </a>
											<?php else :?>
												<a href="/<?=X1?>_search/invest_charm/<?=$row['m_ticker']?>"><?=$row['m_korname']?><span class="ticker"><?=$row['m_ticker']?></span></a>
											<?php endif;?>
                                            </td>
