...
<?php if($this->session->userdata('is_paid')===FALSE && $open_ticker===FALSE) :?>
                <div class="prm_div weeks_free">
                    <div class="box">
                        <div class="left">
                            <p class="title"><i></i>미주미초이스 프리미엄</p>
                            <p class="txt">모든 서비스를 제한없이 이용하실 수 있습니다.</p>
                        </div>
                        <div class="right">
                            <p>
                                <a href="javascript:<?=($this->session->userdata('is_newlink')===TRUE) ? 'fnSinChungNew();':'fnSinChung();'?>" class="btn_free">미주미초이스 신청하기<i></i></a>
                            </p>
                            <p><a href="/<?=KW?>_main/service" class="go_link">[서비스 안내]</a></p>
                        </div>
                    </div>
                </div>
<?php else :?>
...
<?php if($sec_ticker=='' && $pri_ticker=='') {?>
			...
			<?php endif;?>
				...
				<div class="bandchart_sume" id="more">
                    ...
                    <a href="/<?=KW?>_stock/vod_view/93" class="youtube_link"><i></i>매매 타이밍을 쉽게 잡을 수 있는 적정주가 매매전략</a>
                </div>
                <!-- //bandchart_sume -->
            </div>
            <!-- //sub_mid -->
