            ...
            <div class="search_top">
                <div class="data_area">
                    <h2 class="title"><i class="attention<?=($myitem['mi_like']=='Y') ? ' on':''?>" id="catch_icon_one" onClick="javascript:fnMyitem('<?=$ticker['tkr_ticker']?>', '<?=urlencode(str_replace('/index.php/', '', $_SERVER["PHP_SELF"]));?>', 'Y', 'one');"></i><a href="/search/invest_charm/<?=$ticker['tkr_ticker']?>"><?=($sec_ticker != '' && $pri_ticker != '') ? $pri_ticker_name:$ticker['tkr_name']?></a></h2>
                    <ul class="info">
...
<?php //endif;?>
                ...
                <ul class="info_icon">
                    ...
                    <li><a href="/stock/recipe/dividend">배당매력주</a></li>
                    <?php endif; ?>

                    <?php if($mri_data['m_biz_growth_score'] >= 16) : ?>
                    <li><a href="/stock/recipe/growth">이익성장</a></li>
                    <?php endif; ?>

                    <?php if($mri_data['m_biz_moat_score'] >= 16) : ?>
                    <li><a href="/stock/recipe/moat">소비자독점주</a></li>
                    <?php endif; ?>
...
<?php if($sec_ticker=='' && $pri_ticker=='') :?>
            <div class="sub_mid tabs_area">
                <ul class="tabs_5<?=($this->session->userdata('is_paid')===TRUE || $open_ticker===TRUE)? '':' prm_tabs'?>">
                    ...
                    ?>
                    <li class="<?=$tab_active?>"><a href="/search/<?=($seg=='summary')? 'invest_charm' : $seg?>/<?=$ticker_code?>"><?=$tab_tit?></a></li>
                    <?php endforeach; ?>
                </ul>
                <!-- //tabs_5 -->
		...
		<?php if($current_tab == 'summary') : // 종목진단은 내부 텝 3개 공통 상단을 여기서 그려줌 ?>
				<ul class="tabs_3 tabs_depth2">
					...
					<li class="<?=$current_subtab=='summary' ? 'active' : ''?>"><a href="/search/summary/<?=$ticker_code?>">기업개요</a></li>
