            ...
            <div class="sub_mid research_board">
                <div class="video_tabs">
                    <a href="/stock/note" class="">종목노트</a>                    
                    <a href="/stock/research" class="">탐구생활</a>                    
                    <a href="/stock/vod" class="active">동영상</a>                    
                </div>
                
                <div  id="analysis_list" class="lst_type video_list">
					<?php $vmcnt=0; foreach($vod_mjm_list as $key=>$val) :?>
					<?php if($val['vm_display_date'] > date('Y-m-d H:i:s') || $val['vm_view_cs'] !='Y') continue;?>
            ...
            function view_more() {
                ...
                page += 1;
                $.post('/stock/ajax_get_vod_list', {'page': page}, function(res) {
                    if($.trim(res).length) {
