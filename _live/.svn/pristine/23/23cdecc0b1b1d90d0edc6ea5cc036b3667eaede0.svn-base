...
*/
    ...
    public function recommend_view() {
        ...
        $rc_data['rc_invest_point'] = str_replace('https://www.choicestock.co.kr/','https://'.$host.'.choicestock.co.kr/'.HT.'_',$rc_data['rc_invest_point']);
...
//print_r($data['mrt_list']);
    ...
    public function analysis() {
        ...
        $data['content_html'] = $this->load->view('/'.HT.'/stock/analysis_list', $list_data, true);
        $this->header_data['header_template'] = '2';
    ...
    public function ajax_get_analysis_list() {
        ...
        echo $this->load->view('/'.HT.'/stock/analysis_list', $list_data, true);
    ...
    public function recipe($type='dividend') {
        ...
        $data['score_content_html'] = $this->load->view('/'.HT.'/stock/recipe_list.php', array('type' => $type, 'list' => $list), true);
        $data['type'] = $type;
    ...
    public function ajax_get_recipe_list() {
        ...
        $content_html = $this->load->view('/'.HT.'/stock/recipe_list.php', array('type' => $type, 'list' => $list), true);
        echo $content_html;
        return;
    }
    ...
    public function research_view($no=0) {
        ...
        $row['e_content'] = str_replace('https://www.choicestock.co.kr/','https://'.$host.'.choicestock.co.kr/'.HT.'_',$row['e_content']);
    ...
    public function push_control($push='') {
        ...
        if( in_array($push, array('Y', 'N'))) {
            ...
            $this->common->locationhref('/'.HT.'_stock/view_close');
            exit;
        }
        else {
            $this->common->locationhref('/');
                ...
                foreach($ticker_list as $key => $ticker) {
                    ...
                    if($ticker['tkr_ticker']!='') {
                        ...
                        if($choice_ticker != '' && $choice_name != '') {
                            $industry_commnet = '<span class="same_selt"><a href="/'.HT.'_search/invest_charm/'.$choice_ticker.'">'.$choice_name.'</a>와(과) 같은 업종입니다</span>';
                        }
$pre = HT;
$catch_list = <<<PHPSKIN
                ...
                foreach($ticker_list as $key => $ticker) {
                    ...
                    if($ticker['tkr_ticker']!='') {
                        ...
                        if(sizeof($analysis_data) > 0) {
                            ...
                            $ticker_info[$ticker['tkr_ticker']]['analysis_link'] = '/'.HT.'_stock/analysis_view/'.$analy['an_id'];
                        }
                        */
