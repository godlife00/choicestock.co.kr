...
*/
		...
		if( is_file($file_path) ) {
			...
			foreach($morning_list as $morning) {
				if($morning['e_display_date'] <= date('Y-m-d H:i:s')) {
					$cont_banner[$cont]['title'] = $morning['e_title'];//iconv_substr($morning['e_title'], 0, 40, 'utf-8');
					$cont_banner[$cont]['url'] = '/stock/research_view/'.$morning['e_id'];
		...
		if( is_file($file_path) ) {
			...
			foreach($noti_list as $noti) {
				...
				if($noti['nt_view_cs'] == 'Y') {
					if($this->session->userdata('is_paid')===TRUE) {
						$cont_banner[$cont]['title'] = ($noti['nt_ticker_name'] !='' && $noti['nt_ticker'] !='') ? $noti['nt_title'].' - '.$noti['nt_ticker_name'].'('.$noti['nt_ticker'].')':$noti['nt_title'];//iconv_substr($noti['nt_title'], 0, 40, 'utf-8');
					}
					else {
    ...
    public function ajax_save_search_history($srv='C') {
...
//echo '<pre>'; print_r($tab_stock_data);
    ...
    public function service_prm() {
		...
		$data = array();
		$data['is_event'] = $this->is_event;
        $this->_view('/main/service_prm', $data);
    }
    ...
    public function promo_welcome() {
		$data = array();
        $partner_code = $this->input->get('pt');
		if($partner_code == '') $partner_code = 'CSPART019';
		$data['partner_code'] = $partner_code;
        $this->load->view('/mobile/main/service_page', $data);
    }

	public function service_welcome() {
        $this->load->view('/mobile/main/service_welcome');
    }
	...
	public function first_event() {
        $this->load->view('/mobile/main/first_event');
    }
	...
	public function saveTicker() {
		...
		$this->common->locationhref('/main/search');
		exit;
	}

	public function future($ticker='MSFT') {
        if(!strstr($_SERVER['REMOTE_ADDR'], '61.74.181') || IS_REAL_SERVER) {
			return;
		}

		$future_file = 'future.json';
		$file_path = str_replace('hoon','datahero',WEBDATA).'/json/'.$future_file;
		echo $file_path;
		if( is_file($file_path) ) {
			$future_list = json_decode(file_get_contents($file_path), true);
		}

		echo '<pre>'.count($future_list).'<br>'; 
		print_r($future_list[$ticker]);
		//floor($char*100)/100;
		//$future_list['AAOI']['f_total_score'] = floor($future_list['AAOI']['f_total_score']);
		//print_r($future_list['AAOI']);
		//exit;
		//echo '<br><br>'; print_r($future_list);
/*
		...
		echo '<pre>'.count($industry_list); echo '<br><br>'; print_r($industry_list);
*/
	}
}
