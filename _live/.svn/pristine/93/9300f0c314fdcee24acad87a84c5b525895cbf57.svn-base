...
<?php if($sec_ticker=='' && $pri_ticker=='') {?>
				<div class="sub_mid outline_box">
					<div class="box">
						<p class="title"><a href="/<?=WT?>_search/invest_charm/<?=$ticker_code?>">투자매력</a> <span class="txt_guide"><img src="/img/txt_guide.png" alt="가이드보기"></span>
							<div class="guide_box">
						...
						<div class="con">
							...
							<p class="charm_ranking">전체순위 <?=number_format($high_count+1);?>위</p>
					...
					<?php if($fairvalue_rate >= -5) :?>
						...
						<a href="/<?=WT?>_search/fairvalue/<?=$ticker_code?>" class="box">
							<p class="title"><span>적정주가</span></p>
							<div class="con">
						...
						<a href="#" data-modal="modal-3" class="box btn_free md-trigger">
							...
							<div class="con">
								<div class="value_chart">                                                    
									<div class="prm_lock"><span><i></i>미국주식창 신청하기</span></div>
									<div class="line">                  
										<span class="g_action" style="left: 50%;"></span>                                              
									</div>
									<div class="adequate">
				...
				<?php else :?>
					...
					<?php else :?>
						<!--<a href="#" data-modal="modal-3" class="box btn_free md-trigger">-->
						<a href="/<?=WT?>_main/service" class="box">
							<p class="title"><span>적정주가</span></p>
							<div class="con">
								<div class="value_chart">                                                    
									<div class="prm_lock"><span><i></i>미국주식창 신청하기</span></div>
				...
				<?php endif;?>
					...
					<a href="/<?=WT?>_search/invest/<?=$ticker_code?>" class="box">
					...
					<div class="box">
						<p class="title"><a href="<?=($this->session->userdata('is_paid')===TRUE || $open_ticker===TRUE)? '/'.WT.'_stock/recipe_intro':'javascript:;'?>">투자레시피</a></p>
						<div class="con">
							<div class="event_attractiveness">
								<ul>
									<?php $at_count = 0; if($mri_data['m_biz_dividend_score'] >= 16) : ?>
									<li class="att_on"><a href="<?=($this->session->userdata('is_paid')===TRUE || $open_ticker===TRUE)? '/'.WT.'_stock/recipe/dividend':'javascript:;'?>">배당매력주</a></li>
									<?php $at_count++; endif; ?>

									<?php if($mri_data['m_future_score'] >= 16) : ?>
									<li class="att_on"><a href="<?=($this->session->userdata('is_paid')===TRUE || $open_ticker===TRUE)? '/'.WT.'_stock/recipe/growth':'javascript:;'?>">이익성장주</a></li>
									<?php $at_count++; endif; ?>

									<?php if($mri_data['m_biz_moat_score'] >= 16) : ?>
									<li class="att_on"><a href="<?=($this->session->userdata('is_paid')===TRUE || $open_ticker===TRUE)? '/'.WT.'_stock/recipe/moat':'javascript:;'?>">소비자독점주</a></li>
									<?php $at_count++; endif; ?>
					...
					<?php if($this->session->userdata('is_paid')===TRUE || $open_ticker===TRUE) :?>
					<a href="/<?=WT?>_search/competitor/<?=$ticker_code?>" class="box">
					<?php else :?>
					<a href="#" data-modal="modal-3" class="box btn_free md-trigger">
					<?php endif;?>
...
<?php }else{?>
		...
		<div class="sub_mid nondata">
			<p class="nodata_guide"><strong><?=$sec_ticker;?></strong>의 상세 기업정보는 <strong><?php if($part_page == ''){?><a href="/<?=WT?>_search/summary/<?=$pri_ticker?>?pn=<?=$part_name;?>&pg=<?=$part_page;?>"><?php }?><?=$pri_ticker?><?php if($part_page == ''){?></a><?php }?></strong> 종목에서 확인할 수 있습니다.</p>
		</div>
		<!-- //sub_mid nondata -->
<?php }?>
