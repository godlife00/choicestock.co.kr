            ...
            <div class="sub_mid bandchart_more">
                <p><i></i>종목추천&포트폴리오 운영안내 <a href="/stock/recommend/portfolio#more" class="more">자세히</a></p>                
            </div>
                        ...
                        <div class="tab_container">
                            ...
                            <div id="tab3" class="tab_content">
                                ...
                                <table cellspacing="0" border="1" class="tableRanking sum_table">
									...
									<?php $dup_check = array();?>
									<?php $cnt=0; foreach($exclude_list as $key=>$val) :
										$exclude_dup = $val['rc_ticker'].'|'.$val['rc_profit_rate'];
										//if($val['rc_display_date'] > date('Y-m-d H:i:s') || in_array($val['rc_ticker'], $pf_ticker_list) || in_array($exclude_dup, $dup_check)) continue;
			...
			<?php if($tab=='portfolio' && $this->session->userdata('is_paid')===FALSE) :?>
				<?php if(!strstr($_SERVER['HTTP_USER_AGENT'], 'choicestock')) :?>
					<!-- 구글 에드센스 본문 하단 -->
