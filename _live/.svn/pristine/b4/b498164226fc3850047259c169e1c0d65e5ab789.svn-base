			<?php include_once PX_INC_PATH.'/ticker_submenu.php'; ?>
                        ...
                        <div class="swiper-slide">
                            <div class="chart_area" data-id="<?=$tr['rc_id']?>">
                                <div class="chartData left_area">
                                    <h2 class="title" title="<?=$tr['ticker']['tkr_name_en']?>"><a href="/<?=PX?>_stock/recommend_view/<?=$tr['rc_id']?>"><?=$tr['ticker']['tkr_name']?></a></h2>
                                    <ul class="info">
            ...
            <div class="sub_mid eventPicks_area">
				...
				<?php if($tab == '1') : ?>
					...
					<div class="banner_1yearevent">
						...
						<a href="javascript:fnPay();" class="weeks_free">[첫달 900원!]</a>
					</div>
					<?php else :?>
                                ...
                                <table cellspacing="0" border="1" class="tableRanking sum_table">
									...
									?>
                                        <tr>
                                            <td class="name">
											<?php if($this->session->userdata('is_paid')===FALSE) :?>
											<a href="javascript:fnPay();" class="btn_free md-trigger"><?=iconv_substr($val['tkr_name'], 0, 1, 'utf-8')?><span class="remark"><div class="txt_filter size_M"><i></i><i></i><i></i><i></i></div></span><span class="ticker"><span class="remark"><div class="txt_filter size_S"><i></i><i></i><i></i><i></i></div></span></span></a></td>
											<?php else :?>
											<a href="/<?=PX?>_stock/recommend_view/<?=$val['rc_id']?>"><?=$val['tkr_name']?><span class="ticker"><?=$val['rc_ticker']?></span></a></td>
											<?php endif;?>
											<td class="num">
											...
											<?php if($this->session->userdata('is_paid')===FALSE) :?>
											<td class="prm_lock"><span><img src="/img/prm_lock_red.png" alt="잠김"></span></td>
											<?php else :?>
                                ...
                                <table cellspacing="0" border="1" class="tableRanking sum_table">
									...
									?>
                                        <tr>
                                            <td class="name">
											<?php if($this->session->userdata('is_paid')===FALSE) :?>
											<a href="javascript:fnPay();" class="btn_free md-trigger"><?=$val['tkr_name']?><span class="ticker"><?=$val['rc_ticker']?></span></a></td>
											<?php else :?>
											<a href="/<?=PX?>_stock/recommend_view/<?=$val['rc_id']?>"><?=$val['tkr_name']?><span class="ticker"><?=$val['rc_ticker']?></span></a></td>
											<?php endif;?>
				...
				function view_more() {
					...
					page += 1;
					$.post('/<?=PX?>_stock/ajax_get_recommend_list', {'page': page}, function(res) {
						if($.trim(res).length) {
				...
				$(function() {
					$('.sub_top .chart_area').on('click', function() {
						location.href = '/<?=PX?>_stock/recommend_view/' + $(this).data('id');
					});
				});
				</script>
				<?php endif;?>
