...
<?php if($sec_ticker=='' && $pri_ticker=='') {?>
            <div class="search_financials_area">
                <div class="tabs_menu">
                    <span onclick="location.href='/search/financials/<?=$ticker_code?>/MRT';" class="<?=$dimension=='MRT' ? 'active' : ''?>">연환산</span>
                    <span onclick="location.href='/search/financials/<?=$ticker_code?>/MRY';" class="<?=$dimension=='MRY' ? 'active' : ''?>">연간</span>
                    <span onclick="location.href='/search/financials/<?=$ticker_code?>/MRQ';" class="<?=$dimension=='MRQ' ? 'active' : ''?>">분기</span>
                </div>
                <!-- //tabs_menu -->
...
<?php 
				...
				<div class="scroll_table_wrap">
					<!-- 투자지표 table -->
					<h4 class="table_title"><?=$table_name?> <strong class="unit"><?php if(isset($ticker_currency) && $ticker_currency) echo '* 단위 : '.$ticker_currency;?></strong></h4>
					<div class="scroll_table">
						...
						<table cellspacing="0" border="1" class="tableRanking">
							...
							<tbody>
								...
								?>
								<tr<?=($cnt==0 && ($m==0||$m==5||$m==9) || $cnt==1 && ($tit=='자산총계'||$tit=='부채총계'||$tit=='자본총계') || $cnt==2 && ($m==0||$m==1||$m==2)) ? ' class="tr_emphasis"':''?>>
									<td><?=$tit?></td>
					...
					<?php if($cnt<2 && $this->session->userdata('is_paid')===FALSE) :?>
						<?php if(!strstr($_SERVER['HTTP_USER_AGENT'], 'choicestock')) :?>
							<!-- 구글 에드센스 표 상단 -->
							<div style="margin:20px 15px 30px; text-align: center;">                
								<!-- 디스플레이(수평) -->
...
<?php }else{?>
		...
		<div class="sub_mid nondata">
			<p class="nodata_guide"><strong><?=$sec_ticker;?></strong>의 상세 기업정보는 <strong><a href="/search/summary/<?=$pri_ticker?>"><?=$pri_ticker?></a></strong> 종목에서 확인할 수 있습니다.</p>
		</div>
		<!-- //sub_mid nondata -->
<?php }?>