            ...
            <div class="sub_top">
                <div class="">
                    <div class="txt_box">
                        <p class="txt">                            
                            투자포인트, 추천가, 목표가, 매도가를 포함한 <strong>매매전략을 제공</strong>합니다. 종목추천 주기는 주 <strong>5회 (월~금)</strong>입니다.<br>
            ...
            <div class="sub_mid bandchart_more">
                <p><i></i>종목추천&포트폴리오 운영안내 <a href="#more" class="more">자세히</a></p>                
            </div>
            ...
            <div class="sub_mid eventPicks_area">
                                ...
                                <table cellspacing="0" border="1" class="tableRanking sum_table">
                                    ...
                                    ?>
                                        <tr>
                                            ...
                                            <a href="#" data-modal="modal-3" class="btn_free md-trigger"><?=iconv_substr($val['tkr_name'], 0, 1, 'utf-8')?><span class="remark"><div class="txt_filter size_M"><i></i><i></i><i></i><i></i></div></span><span class="ticker"><span class="remark"><div class="txt_filter size_S"><i></i><i></i><i></i><i></i></div></span></span></a></td>
                                            <?php else :?>
                                            <a href="/<?=WT?>_stock/recommend_view/<?=$val['rc_id']?>"><?=$val['tkr_name']?><span class="ticker"><?=$val['rc_ticker']?></span></a></td>
                                            <?php endif;?>
                                            <td class="num">
                                            ...
                                            <?php if($this->session->userdata('is_paid')===FALSE && $cnt>1) :?>
                                            <td class="prm_lock"><span><img src="/img/prm_lockwowtv.svg" alt="잠김"></span></td>
                                            <?php else :?>
                                ...
                                <table cellspacing="0" border="1" class="tableRanking sum_table">
                                    ...
                                    ?>
                                        <tr<?=($val['rc_profit_rate']<0) ? ' class="hide_line"':''?>>
                                            ...
                                            <a href="#" data-modal="modal-3" class="btn_free md-trigger"><?=$val['tkr_name']?><span class="ticker"><?=$val['rc_ticker']?></span></a></td>
                                            <?php else :?>
                                            <a href="/<?=WT?>_stock/recommend_view/<?=$val['rc_id']?>"><?=$val['tkr_name']?><span class="ticker"><?=$val['rc_ticker']?></span></a></td>
                                            <?php endif;?>
                ...
                function view_more() {
                    ...
                    page += 1;
                    $.post('/<?=WT?>_stock/ajax_get_recommend_list', {'page': page}, function(res) {
                        if($.trim(res).length) {
                ...
                $(function() {
                    $('.sub_top .chart_area').on('click', function() {
                        location.href = '/<?=WT?>_stock/recommend_view/' + $(this).data('id');
                    });
                });
                </script>
                <?php endif;?>
                ...
                <div class="banner_prm">
                    <span class="cho_prm"><i></i>미국주식창 프리미엄</span>
                    <a href="#" data-modal="modal-3" class="weeks_free btn_free md-trigger">[ 첫 달 3,300원! ]</a>
                </div>
                <?php endif;?>
				...
				<div class="bandchart_sume" id="more">
					...
					<div class="box info">
						<p><i></i>‘종목추천&포트폴리오’는 (주)데이터히어로의 금융 빅 데이터 분석과 알고리즘에 따라 저평가 우량주를 최적의 시점에 매매할 수 있도록 도움을 드리는 서비스입니다.</p>
					</div>
					<!-- //box -->
