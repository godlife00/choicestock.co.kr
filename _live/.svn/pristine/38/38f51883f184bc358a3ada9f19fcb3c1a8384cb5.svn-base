...
class Search extends BaseMobile_Controller {
    ...
    private function _general_data($ticker, $dimension='MRY', $option=array()) {
        ...
        $data['mri_data']['m_future_stars'] = @sprintf('%0.1f', $data['mri_data']['m_future_stars']);            
        //echo '<pre>'; print_r($data['mri_data']); exit;
        //$data['mri_data']['m_v_fairvalue5'] = @sprintf('%0.2f', $data['mri_data']['m_v_fairvalue3']*0.591); //매우저평가
        //$data['mri_data']['m_v_fairvalue4'] = @sprintf('%0.2f', $data['mri_data']['m_v_fairvalue3']*0.769); //저평가
        //$data['mri_data']['fairvalue3'] = $mri['valuation']['fairvalue3'];                        //적정가    
        //$data['mri_data']['m_v_fairvalue2'] = @sprintf('%0.2f', $data['mri_data']['m_v_fairvalue3']*1.3);    //고평가
        //$data['mri_data']['m_v_fairvalue1'] = @sprintf('%0.2f', $data['mri_data']['m_v_fairvalue3']*1.69); //매우고평가
        ...
        if($this->is_open === true) {
            $data['ticker'] = $this->ticker_tb_model->convertWowInfo($data['ticker']);
        }

        //echo '<pre>';print_r($data['ticker']); exit;

        //관심종목 체크
        $data['myitem'] = $this->like($ticker, 'Y');

        //2021.03.21 추가
          $snp500_ticker = array();
        $snp500_ticker = $this->get_snp500();

        $data['is_snp500'] = false;
        if(in_array($ticker, $snp500_ticker)) $data['is_snp500'] = true;

        //배당락일 가져오기
        $exdate = @array_shift($this->itoozaapi->getExdate($ticker));

        if($exdate!='') {
            $nyear = date('Y');
            $eyear = substr($exdate, 0, 4);
            $term = $nyear - $eyear;
            if($term>10) $exdate = '';
        }
        $data['exdate'] = $exdate;
        ...
        $data['report_date'] = @array_shift(($this->sf1_tb_model->getList($params, $extra)->getData()));

        if( isset($option['get_band_list']) && $option['get_band_list']) {
            $this->load->model(DBNAME.'/mriall_tb_model');
            $params = array();
            $params['=']['m_ticker'] = $ticker;
            $params['>=']['m_sep_date'] = '20200531';
            $params['join']['sep_tb'] = 'm_ticker = sep_ticker and m_sep_date = sep_date';

            $extra = array(
                    'fields' => 'm_sep_date, sep_close, m_v_fairvalue5, m_v_fairvalue4, m_v_fairvalue3, m_v_fairvalue2, m_v_fairvalue1, m_disparate ',
            ...
            $data['band_list'] = array_reverse($this->mriall_tb_model->getList($params, $extra)->getData());
        }
        ...
        if( strtoupper($data['ticker']['tkr_exchange']) == 'NASDAQ' || strtoupper($data['ticker']['tkr_exchange']) == 'NYSE' || strtoupper($data['ticker']['tkr_exchange']) == 'NYSEMKT' ) {
            if( strstr(strtoupper($data['ticker']['tkr_category']), 'PREFERRED') || strstr(strtoupper($data['ticker']['tkr_category']), 'SECONDAR') ) {
                ...
                if(isset($pri_ticker) && is_array($pri_ticker)) {
                    ...
                    $data['pri_ticker_name'] = (in_array($ticker, $this->exc_ticker)) ? $this->ticker_info_map[$ticker]['tkr_name']:$this->ticker_info_map[$pri_ticker[0]['tkr_ticker']]['tkr_name'];
                }
            }
        }
    ...
    public function news($ticker='') {
        ...
        $data['meta_title'] = $data['ticker']['tkr_name'].' '.$ticker.'('.$data['ticker']['tkr_name_en'].') | 뉴스 - 초이스스탁US';

        $this->header_data['back_url'] = '-1';

        $this->load->model(DBNAME.'/news_idx_tb_model');

        $params['=']['ni_code'] = SRV_CODE;
        //$params['=']['ni_is_active'] = 'YES';
        $params['=']['ni_view_cs'] = 'Y';
        $params['<=']['ni_display_date'] = date('Y-m-d H:i:s');
        $params['=']['ni_ticker'] = $ticker;
        //$params['not in']['ni_view_srv'] = array('W', 'M'); ; //월가(W), 미주미(M)제외, 전체(''). 초이스(C)
        //$params['=']['ni_is_inside'] = 'N'; //외부표출금지(Y/N) 초스만 표출
        //$params['!=']['ni_kiwoom_off'] = 'Y'; //키움표출금지(Y/N) 키움은 미표출
        $extra = array(
            'fields' => 'ni_id, ni_table, ni_title, ni_display_date, ni_thumbnail, ni_writer_name, ni_pay',
            'order_by' => 'ni_display_date DESC',
            'limit' => '30',
            'slavedb' => true
        );
        $data['news_list'] = $this->news_idx_tb_model->getList($params, $extra)->getData();
        $this->_view('/search/news', $data);
    }
    ...
    public function ajax_get_news_list() {
        ...
        $list_data['news_list'] = $this->news_idx_tb_model->getList($params, $extra)->getData();
        $list_data['ticker'] = $ticker;
        //$list_data['is_event'] = $this->is_event;
        echo $this->load->view('/mobile/search/news_list', $list_data, true);
    }

    public function news_view($ticker='', $no=0) {

        $this->load->model(DBNAME.'/news_idx_tb_model');
        $this->load->model(DBNAME.'/explore_tb_model');
        $this->load->model(DBNAME.'/vod_mjm_tb_model');
        $this->load->model(DBNAME.'/news_tb_model');

        $data = array();

        if( ! $this->news_idx_tb_model->get($no)->isSuccess()) {
            $this->common->historyback();
            return;
        }

        $row = $this->news_idx_tb_model->getData();
       
        //if($row['ni_display_date'] > date('Y-m-d H:i:s') || $row['ni_is_active'] == 'NO' || ! in_array($row['ni_table'], array('news_tb', 'vod_mjm_tb', 'explore_tb'))) { 
        if($row['ni_display_date'] > date('Y-m-d H:i:s') || $row['ni_view_cs'] != 'Y' || ! in_array($row['ni_table'], array('news_tb', 'vod_mjm_tb', 'explore_tb'))) { 
            $this->common->historyback();
            return;
        }
        ...
        if( ! $this->{$row['ni_table'].'_model'}->get($row['ni_pk'])->isSuccess()) {
            $this->common->historyback();
            return;
        }

        $news = $this->{$row['ni_table'].'_model'}->getData();

        if($row['ni_table']=='explore_tb') {
            $data['title'] = $news['e_title'];
            $data['content'] = $this->auto_link($news['e_content']);
            $data['display_date'] = $news['e_display_date'];
            $data['writer_name'] = '';
            $id = $news['e_id'];
            $count_column = 'e_view_count';
        }
        else if($row['ni_table']=='vod_mjm_tb') {
            $data['title'] = $news['vm_title'];
            $data['content'] =  $this->auto_link($news['vm_content']);
            $data['display_date'] = $news['vm_display_date'];
            $data['writer_name'] = '';
            $id = $news['vm_id'];
            $count_column = 'vm_view_count';
        }
        else {
            $data['title'] = $news['n_title'];
            $data['content'] =  $this->auto_link($news['n_content']);
            $data['display_date'] = $news['n_display_date'];
            $data['writer_name'] = $news['n_writer_name'];
            $id = $news['n_id'];
            $count_column = 'n_view_count';
        }
        ...
        $this->{$row['ni_table'].'_model'}->doUpdate($id, $update_params, array("{$count_column}"));
        //$this->{$row['ni_table'].'_model'}->doUpdate($news['n_id'], $update_params, array('n_view_count'));
        
        //$this->header_data['header_template'] = '4';
        $this->header_data['head_title'] = '뉴스';
        $this->header_data['back_url'] = '/search/news/'.$ticker;
        //$this->header_data['back_url'] = '/';
        $data['meta_title'] = $row['n_title'].' | 뉴스 - 초이스스탁US';
        //$data['meta_title'] = $data['ticker']['tkr_name'].' '.$ticker.'('.$data['ticker']['tkr_name_en'].') | 뉴스 - 초이스스탁US';

        $this->_view('/search/news_view', $data);
    }

    // 종목검색-적정주가(2021.3.17추가)
    public function fairvalue($ticker='AAPL') {
        $ticker = strtoupper($ticker);
        ...
        $data['current_tab'] = 'invest_charm';
        $data['current_subtab'] = 'fairvalue'; // invest_charm | summary | finance_chart
        ...
        $data['is_fairvalue'] = true;
        $data['ticker_header'] = $this->load->view('/mobile/search/ticker_header', $data, true);

        //적정가 차트 생성용
        $data['fairvalue_rate'] = $this->get_fairrate($data['ticker']['tkr_close'], $data['mri_data']);

        $data['meta_title'] = $data['ticker']['tkr_name'].' '.$ticker.'('.$data['ticker']['tkr_name_en'].') | 적정주가 - 초이스스탁US';

        $this->header_data['back_url'] = '-1';
        $data['is_event'] = $this->is_event;

        $this->_view('/search/fairvalue', $data);
    }

    // 종목검색-개요(2021.3.12추가)
    public function summary($ticker='AAPL') {
        ...
        //$data['current_subtab'] = 'outline'; // invest_charm | summary | finance_chart
        ...
        $extra['order_by'] = '';
        $extra['fields'] = array(
            'avg(m_biz_total_score) as m_biz_total_score', 
        );
        $extra['slavedb'] = true;
...
//$data['ticker_unit'] = 0;
        ...
        // 경쟁사 채우기
        $data['competitor_data'] = $this->itoozaapi->getCompetitor($ticker)?:array();

        $data['meta_title'] = $data['ticker']['tkr_name'].' '.$ticker.'('.$data['ticker']['tkr_name_en'].') | 개요 - 초이스스탁US';
        ...
        if(isset($chart[$ticker]) && is_array($chart[$ticker])) {
            ...
            $close_20 = array_values(array_slice($chart[$ticker], (-20*1), 1)); //20일 전 종가

            $data['rate_of_rise'] = round((($data['ticker']['tkr_close'] - $close_20[0])/$close_20[0])*100,2);
        }
...
//최근1개월 상승률 = {(전일종가-20일전종가)/20일전종가}x100%
//최근1개월 간 = {(전일종가-20일전종가)/20일전종가}x100%, 상승률 상위 50종목
//최근3개월 간 = {(전일종가-60일전종가)/60일전종가}x100%, 상승률 상위 50종목

        $this->_view('/search/outline', $data);
     }

    // 종목진단 - 투자매력
    public function invest_charm($ticker='AAPL', $dimension='MRY') {
        ...
        $file_path = str_replace('hoon','datahero',WEBDATA).'/json/'.$industry_file;

        if( is_file($file_path) ) {
            $industry_list = json_decode(file_get_contents($file_path), true);
        }

        $data['mri_data']['f_sales_indu'] = str_replace('%', '', $industry_list[$data['ticker']['tkr_industry']]['f_sales']);
        $data['mri_data']['f_sales_all'] = str_replace('%', '', $industry_list['ALL']['f_sales']);
        $data['mri_data']['f_profit_indu'] = str_replace('%', '', $industry_list[$data['ticker']['tkr_industry']]['f_profit']);
        $data['mri_data']['f_profit_all'] = str_replace('%', '', $industry_list['ALL']['f_profit']);
        /////////////////////////////// 종목진단 공통
        // 전체 종목 수
        $params = array();
        $params['slavedb'] = true;
...
*/
        ...
        $extra['order_by'] = '';
        $extra['fields'] = array(
            'avg(m_future_stars) as m_future_stars', 
            ...
            'avg(m_g_netmargin) as m_g_netmargin', 
            'avg(m_g_roa) as m_g_roa', 
            'avg(m_g_roic) as m_g_roic', 
        ...
        $data['indavg_scores'] = array_shift($this->mri_tb_model->getList($params, $extra)->getData());
        $data['indavg_scores'] = array_map('floatval', $data['indavg_scores']);
//echo '<pre>'; print_r($data['indavg_scores']);//exit;

        //$data['mri_detail'] = $this->itoozaapi->getBizMRIDetail($ticker);

        // 배당매력 상위 퍼센트 => 미래성장성 상위 퍼센트(2021.08.25 변경)
        $params = array();
        //$params['>=']['m_biz_dividend_score'] = $data['mri_data']['m_biz_dividend_score'];
        $params['=']['m_future_stars'] = 5;
        $params['slavedb'] = true;
        $params['cache_sec'] = 3600*24;
        $mri_high_count = $this->mri_tb_model->getCount($params)->getData();
        $data['future_high_count'] = $mri_high_count;
        $data['future_high_rank_rate'] = number_format($mri_high_count / $mri_count * 100, 2);

        //문구 가져오기
        $params = array();
        $params['total'] = $mri_count;
        $params['high_rate'] = $data['future_high_rank_rate'];
        $params['high'] = $mri_high_count;
        $sp=0;
        $sp=floor($data['mri_data']['m_future_stars']);
        if($sp<1) $sp = 1;
        $data['mri_data']['future'] = $this->get_spcomment('future_'.$sp, $params);

//echo 'indavg_scores===>'.$data['indavg_scores']['m_future_stars'].'<br>';
//echo 'avg_scores===>'.$data['avg_scores']['m_future_stars'];

        $params = array();
        $params['>']['m_future_score'] = ($data['mri_data']['m_future_score']=='') ? '0' : $data['mri_data']['m_future_score'];
        $params['slavedb'] = true;
        $params['cache_sec'] = 3600*24;
        $mri_future_count = $this->mri_tb_model->getCount($params)->getData();
        $data['future_rank_rate'] = number_format( ($mri_future_count+1) / $mri_count * 100, 2);
        ...
        //한달 전 투자매력 점수 가져오기
        $pre_bizscore = $this->_get_pre_mriscore($ticker, $data['mri_data']['m_v_fairvalue3']);
        $data['pre_mriscore'] = $pre_bizscore['m_biz_total_score'];
        $data['pre_fairvalue'] = $pre_bizscore['pre_fairvalue'];
        $data['pre_sepdate'] = $pre_bizscore['pre_sepdate'];
        //적정가 차트 생성용
        $data['fairvalue_rate'] = $this->get_fairrate($data['ticker']['tkr_close'], $data['mri_data']);
...
/*차트*/
        ...
        //echo '<pre>'; print_r($competitor_tickers); exit;
        if(sizeof($competitor_tickers) > 0) {
            $params = array();
            $params['in']['m_ticker'] = $competitor_tickers;
            $extra = array(
                'fields' => 'm_ticker, m_korname, m_usname, m_biz_total_score, m_future_score, m_biz_moat_score, m_biz_growth_score, m_biz_safety_score, m_biz_cashflow_score, m_future_stars, m_biz_moat_stars, m_biz_growth_stars, m_biz_safety_stars, m_biz_cashflow_stars',
                'order_by' => ''
            );
            $extra['slavedb'] = true;
            $extra['cache_sec'] = 3600*24;
        ...
        $data['meta_title'] = $data['ticker']['tkr_name'].' '.$ticker.'('.$data['ticker']['tkr_name_en'].') | 투자매력 - 초이스스탁US';
    ...
    private function _get_pre_mriscore($ticker, $fair_value=0) {
        $this->load->model(DBNAME.'/mriall_tb_model');
        ...
        $result = array_shift($this->mriall_tb_model->getList($params, $extra)->getData());

        if($fair_value>0) {
            $params = array();
            $params['=']['m_ticker'] = $ticker;
            $params['!=']['m_v_fairvalue3'] = $fair_value;

            $extra = array(
                'fields' => 'm_sep_date, m_v_fairvalue3',
                'order_by' => 'm_sep_date desc',
                'limit' => 1,
                'quandldb' => true,
                'cache_sec' => 3600*2
            );
            $pre_data = array_shift($this->mriall_tb_model->getList($params, $extra)->getData());
            $result['pre_sepdate'] = $pre_data['m_sep_date']; 
            $result['pre_fairvalue'] = $pre_data['m_v_fairvalue3']; 
        }
    ...
    // 종목진단 - 경쟁사
    public function competitor($ticker='AAPL') {
        ...
        //$data['is_open'] = $this->is_open;
        $data['ticker_header'] = $this->load->view('/mobile/search/ticker_header', $data, true);
        $data['meta_title'] = $data['ticker']['tkr_name'].' '.$ticker.'('.$data['ticker']['tkr_name_en'].') | 경쟁사 - 초이스스탁US';

        $this->_view('/search/competitor', $data);
    }

    public function mkChart($ticker='AAPL', $dimension='MRY') {
        $ticker = strtoupper($ticker);
        $ticker_info = $this->ticker_info_map[$ticker];

        //2021.01.06 수정
        $is_adr = false;
        if( strstr(strtoupper($ticker_info['tkr_category']), 'ADR') || strstr(strtoupper($ticker_info['tkr_category']), 'CANADIAN') ) {
            $is_adr = true;
        }
		$data = array();
        $data = $this->itoozaapi->getCharmFinanceVChartFile($ticker, $dimension, $is_adr);

		echo serialize($data); 
		exit;
	}

    public function finance_chart($ticker='AAPL', $dimension='MRY') {
        $ticker = strtoupper($ticker);
        if( ! isset($this->ticker_info_map[$ticker])) {
            $this->common->locationhref('/');
            return;
        ...
        $data['vchart_data'] = $this->itoozaapi->getCharmFinanceVChartFile($ticker, $dimension, $is_adr);
        //echo '<pre>'; print_r($data['vchart_data']); exit;
        //$data['vchart_data'] = $this->itoozaapi->getCharmFinanceVChart($ticker, $dimension, '', $is_adr);
        ...
        */
        ...
        $data['meta_title'] = $data['ticker']['tkr_name'].' '.$ticker.'('.$data['ticker']['tkr_name_en'].') | 재무차트 - 초이스스탁US';
...
*/
    // 재무제표
    public function financials($ticker='AAPL', $dimension='MRT') {
        $ticker = strtoupper($ticker);
        ...
        $data['current_tab'] = 'financials';
        $data['current_subtab'] = 'financials';
        ...
        */
        ...
        $field_title_map['finstate']['sf1_taxassets'] = '이연세자산';
        $field_title_map['finstate']['sf1_assetsetc'] = '기타자산';
        ...
        $field_title_map['finstate']['sf1_taxliabilities'] = '이연세부채';

        if($is_second === true) {
            $field_title_map['finstate']['sf1_deposits'] = '예수부채';
        }

        $field_title_map['finstate']['sf1_liabilitiesetc'] = '기타부채';
    ...
    // 투자지표
    public function invest($ticker='AAPL', $dimension='MRT') {
        $ticker = strtoupper($ticker);
        $dimension = strtoupper($dimension);
        if( ! isset($this->ticker_info_map[$ticker])) {
            $this->common->locationhref('/');
    ...
    // 배당 
    public function alloca($ticker='AAPL') {
        $this->common->locationhref('/search/dividend/'.$ticker);
        exit;
    }

    public function dividend($ticker='AAPL') {
        $ticker = strtoupper($ticker);
        if( ! isset($this->ticker_info_map[$ticker])) {
            $this->common->locationhref('/');
            return;
        ...
        $data['meta_title'] = $data['ticker']['tkr_name'].' '.$ticker.'('.$data['ticker']['tkr_name_en'].') | 배당 - 초이스스탁US';
        $data['is_event'] = $this->is_event;
...
/*
    private function _like($ticker='') {
        ...
        if( $this->session->userdata('is_login') === TRUE && isset($ticker)) {
            ...
            return array_pop($this->myitem_tb_model->getList($params, $extra)->getData());
...
*/
    public function setTicker($ticker='', $count='') {
        ...
        if( $this->session->userdata('is_login') === true ) {
            ...
            $result = array('error' => '로그인 후 이용해주세요.', 'res' => '1');
            exit(json_encode($result));
        }
    }

	public function wall_api() {
	                  
		//$file_path = 'https://hero-shinhan.datahero.co.kr/stock/api/cardnews?api_key=K_zTp4dxld30uyEEe08GO7&pg=1';
		$file_path = 'https://usdev.datahero.co.kr/stock/api/cardnews?api_key=K_zTp4dxld30uyEEe08GO7&pg=1';
		//$file_path = 'https://www.quandl.com/api/v3/datatables/SHARADAR/daily.json?date.gte=2021-10-12&api_key=b9Q9E2d-D3EMyH7FQ9qE';
    ...
    public function api($type='') {
        ...
        if($type=='A') {
            ...
            $result['growth'] = '';
            $result['grlink'] = '';
            if($data['mri_data']['m_future_score'] >= 16) {
                $result['growth'] = '성장주';
            ...
            if($this->is_open === true) {                
                $result['date'] = ($data['ticker']['tkr_status'] == '1') ? date('y.m/d', strtotime($data['ticker']['tkr_lastpricedate'])):date('y.m/d H:i', strtotime($data['ticker']['tkr_lastpricedate']));    // 10)기준일
            }
            else {
            ...
            if( is_file($file_path) ) {
                ...
                if($rand == '1') {
                    ...
                    $explore = array_shift($this->explore_tb_model->getList($params, $extra)->getData());
                    $result['extitle'] = str_replace('<br>',' ',$explore['e_title']); // 23) 탐구생활 제목
                    $result['exlink'] =  'https://www.choicestock.co.kr/stock/research_view/'.$explore['e_id'];  // 23) 탐구생활 링크
					*/

					// 2021.09.03 수정
                    $result['extitle'] = str_replace('<br>',' ',$file_data['bn'.$rand.'_txt']); // 23) 동영상 제목
                    $result['exlink'] =  'https://www.choicestock.co.kr/stock/vod_view/'.$file_data['bn'.$rand.'_id'];  // 23) 동영상 링크
                }
                else if($rand == '2') {
                    ...
                    $result['extitle'] = '어제 급등주는? 저평가 우량주? 투자레시피에서 찾자'; 
                    $result['exlink'] =  'https://www.choicestock.co.kr/stock/recipe_intro';
                }
            }
            ...
            $ticker_info_file = 'ticker_api.json';
            //$file_path = WEBDATA.'/'.$ticker_info_file;
            $file_path = str_replace('hoon','datahero',WEBDATA).'/json/'.$ticker_info_file;

            if( is_file($file_path) ) {
                $ticker_list = json_decode(file_get_contents($file_path), true);
                ...
                $result = array();
                $result['retmsg'] = 'ticker info error[file not exist]';
                echo json_encode($result);
                //    $strJsonStr = json_encode($stockbot_array);
                exit;
            }
            ...
            $winner = 'winner_all.json';
            //$file_path = WEBDATA.'/'.$ticker_info_file;
            $file_path = str_replace('hoon','datahero',WEBDATA).'/'.$winner;

            if( is_file($file_path) ) {
				$win_socre = array('S'=>20, 'N'=>5, 'W'=>1); 
				$win_total_socre = array('60'=>100,'45'=>90,'41'=>80,'30'=>70,'26'=>60,'22'=>50,'15'=>40,'11'=>30,'7'=>20,'3'=>10); 

                $winner = json_decode(file_get_contents($file_path), true);
				$winner = $winner[$ticker];
				$ticker_score = $win_total_socre[$win_socre[$winner['win_short']]+$win_socre[$winner['win_medium']]+$win_socre[$winner['win_long']]];

				$result = array();
				$result['ticker'] = $ticker;
				$result['short'] = $winner['win_short'];
				$result['medium'] = $winner['win_medium'];
				$result['long'] = $winner['win_long'];
				$result['score'] = $ticker_score;
                echo json_encode($result);
                exit;
			}
			else {
			...
			//snp500 https://capdev.choicestock.co.kr/search/api/F?api_key=K_zTp4dxld30uyEEe08GO7&ticker=LA
			$result = array();
	        $result = $this->get_snp500();
			echo json_encode($result);
			exit;
		}
        else if($type=='G') {
            $path = str_replace('datahero','hoon',WEBDATA).'/master_data/consensus.inc';

			$consensus = array();
			//ticker	컨센서스 기준	매출(sales)	EPS	ROE
			if(file_exists($path)) {
				$contents = explode("\n", file_get_contents($path));
				foreach($contents as $idx => $row) {
					$row = explode("\t", $row);
					if($idx>0 && $row[0] != '') {
						$consensus[$row[0]]['year'] = $row[1];
						$consensus[$row[0]]['sales'] = $row[2];
						$consensus[$row[0]]['eps'] = $row[3];
						$consensus[$row[0]]['roe'] = $row[4];
					}
				}
    ...
    function sitemap() {
        ...
        $ticker_list = $this->ticker_info_map;
        foreach($ticker_list as $key=>$val) {
            $str = '<url><loc>https://www.choicestock.co.kr/search/invest_charm/'.$val['tkr_ticker'].'</loc><changefreq>always</changefreq><priority>0.85</priority></url>';
            echo htmlspecialchars($str).'<br>';
        }
    }

    public function getSpiderData($date='') {
        $this->loginCheck();
        ...
        if($date=='') {
            ...
            $mri_list = $this->mriall_tb_model->getList($params, $extra)->getData();
        }

        //echo '<strong>날짜 : '.date('Y년 m월 d일', strtotime($date)).'<strong>'; 

        echo '<table border=1>';
        echo '<tr><td>Ticker</td><td>Korname</td><td>Exchange</td><td>Sector</td><td>Industry</td><td>Biz Margin Score</td><td>Biz Safety Score</td><td>Biz Cashflow Score</td><td>Biz Moat Score</td><td>Future Score</td><td>Biz Dividend Score</td><td>Biz Total Score</td><td>Star Margin Score</td><td>Star Safety Score</td><td>Star Cashflow Score</td><td>Star Moat Score</td><td>Star Future Score</td><td>Star Dividend Score</td><td>Star Total Score</td><td>ranking</td><td>highrank</td><td>Sep Date</td><td>Close</td><td>Marketcap</td><td>Pb</td><td>Pe</td><td>Ps</td><td>Roe</td><td>Divyield</td><td>Date</td></tr>';
        foreach($mri_list as $key=>$val) {
            echo '<tr><td>'.$val['m_ticker'].'</td><td>'.$val['m_korname'].'</td><td>'.$val['m_exchange'].'</td><td>'.$val['m_sector'].'</td><td>'.$val['m_industry'].'</td><td>'.$val['m_biz_growth_score'].'</td><td>'.$val['m_biz_safety_score'].'</td><td>'.$val['m_biz_cashflow_score'].'</td><td>'.$val['m_biz_moat_score'].'</td><td>'.$val['m_future_score'].'</td><td>'.$val['m_biz_dividend_score'].'</td><td>'.$val['m_biz_total_score'].'</td><td>'.$val['m_biz_growth_stars'].'</td><td>'.$val['m_biz_safety_stars'].'</td><td>'.$val['m_biz_cashflow_stars'].'</td><td>'.$val['m_biz_moat_stars'].'</td><td>'.$val['m_future_stars'].'</td><td>'.$val['m_biz_dividend_stars'].'</td><td>'.($val['m_biz_growth_stars']+$val['m_biz_safety_stars']+$val['m_biz_cashflow_stars']+$val['m_biz_moat_stars']+$val['m_future_stars']).'</td><td>'.$val['m_ranking'].'</td><td>'.$val['m_highrank'].'</td><td>'.$val['m_sep_date'].'</td><td>'.$val['m_close'].'</td><td>'.$val['m_marketcap'].'</td><td>'.$val['m_pbr'].'</td><td>'.$val['m_per'].'</td><td>'.$val['m_psr'].'</td><td>'.$val['m_roe'].'</td><td>'.$val['m_divyield'].'</td><td>'.$val['m_date'].'</td></tr>';
        }
        echo '</table>';
    }
}
