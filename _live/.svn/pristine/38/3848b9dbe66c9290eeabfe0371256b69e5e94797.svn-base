...
<?php endif;?>
								<?php if($type == 'earnings') :?>
									<?php $in_cnt=0; $cnt=1;foreach($list['recent_report'] as $key => $val) : ?>
										<div class="area">
											<ul class="list">
												<li class="title"><a href="/search/summary/<?=$val['tkr_ticker']?>"><?=$val['tkr_name']?></a></li>
												<li class="num">
										...
										<?php if($cnt==6 || $cnt==14) :?>
											<?php if($this->session->userdata('is_paid')===FALSE) :?>
												<!-- 구글AD -->
												<?php include INC_PATH.'/list_google.php';?>
												<!-- //구글AD -->
											<?php endif;?>
										<?php endif;?>

									<?php $in_cnt++; $cnt++; endforeach;?>
								<?php elseif($type == 'interest') :?>
									<?php $in_cnt=0; $cnt=1; foreach($list as $val) :?>
									<div class="area">
										<ul class="list">
											<li class="title">
												<a href="/search/summary/<?=$val['m_ticker']?>"><?=$val['ticker']['tkr_name']?></a>
									...
									<div class="tab_container recipejump_tabs">
										...
										<div id="tab1" class="tab_content">
											...
											<div class="recipe_area">
												<?php $cnt=1; foreach($list['one'] as $val) :?>
												<div class="area">
													<ul class="list">
														<li class="title">
															<a href="/search/summary/<?=$val['m_ticker']?>"><?=$val['ticker']['tkr_name']?></a>
										...
										<div id="tab2" class="tab_content">
											<p class="update_info">업데이트 <?=$list['update']?>, <?=$list['subtitle'];?></p>
											<div class="recipe_area">
												<?php $cnt=1; foreach($list['twenty'] as $val) :?>
												<div class="area">
													<ul class="list">
														<li class="title">
															<a href="/search/summary/<?=$val['m_ticker']?>"><?=$val['ticker']['tkr_name']?></a>
										...
										<div id="tab3" class="tab_content">
											<p class="update_info">업데이트 <?=$list['update']?>, <?=$list['subtitle'];?></p>
											<div class="recipe_area">
												<?php $cnt=1; foreach($list['sixty'] as $val) :?>
												<div class="area">
													<ul class="list">
														<li class="title">
															<a href="/search/summary/<?=$val['m_ticker']?>"><?=$val['ticker']['tkr_name']?></a>
								...
								<?php elseif($type == 'sale') :?>
									...
									<div class="area">
										<ul class="list">
											...
											<?php if($this->session->userdata('is_paid')===TRUE) :?>
												<a href="/search/summary/<?=$val['tkr_ticker']?>"><?=$val['tkr_name']?></a>
												<span class="sum"><?=$val['tkr_ticker']?></span>
											<?php else :?>										
												<a href="#" data-modal="<?=($is_event === true) ? 'modal-4':'modal-3'?>" class="btn_free md-trigger"><?=iconv_substr($val['tkr_name'], 0, 1, 'utf-8')?><span class="remark"><div class="txt_filter size_M"><i></i><i></i><i></i><i></i></div></span></a>
												<span class="sum">
									...
									<?php $in_cnt++; $cnt++; endforeach;?>
								<?php else :?>
									<?php $in_cnt=0; $cnt=1; foreach($list as $key => $val) : ?>
									<?php 
										$stars = 0;
										$price = 0;
										if($type == 'dividend') {
									...
									<div class="area">
										<ul class="list">
											<?php if($this->session->userdata('is_paid')===FALSE && $type == 'dividend') :?>
												<li class="title">
													<a href="#" data-modal="modal-3" class="md-trigger">
											...
											<?php else :?>										
												<li class="title"><a href="/search/summary/<?=$val['m_ticker']?>"><?=$val['m_korname']?></a></li>
											<?php endif;?>
											<li class="num">
