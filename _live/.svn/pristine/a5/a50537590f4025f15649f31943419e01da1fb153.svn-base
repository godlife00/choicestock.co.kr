            <?php include_once KW_INC_PATH.'/ticker_submenu.php'; ?>
            ...
            <div class="sub_mid bandchart_more">
                <p><i></i>종목추천&포트폴리오 운영안내 <a href="/<?=KW?>_stock/recommend/portfolio#more" class="more">자세히</a></p>                
            </div>
            ...
            <div class="sub_mid eventPicks_area">
                ...
                <?php if($tab == '1') : ?>
                    ...
                    <!--mjm <div class="banner_1yearevent">
                        <span class="cho_prm"><i></i>미주미초이스 프리미엄</span>
                    ...
                    <!--mjm <div class="banner_prm">
                        <span class="cho_prm"><i></i>미주미초이스 프리미엄</span>
                        <a href="/<?=KW?>_main/service_free" class="weeks_free">[첫달 3,000원!]</a>
                    </div>-->
                    <?php endif;?>
                    <?php endif;?>
                <?php endif;?>
                                ...
                                <table cellspacing="0" border="1" class="tableRanking sum_table">
                                    ...
                                    ?>
                                        <tr>
                                            ...
                                            <a href="javascript:<?=($this->session->userdata('is_newlink')===TRUE) ? 'fnSinChungNew();':'fnSinChung();'?>" class="btn_free md-trigger"><?=iconv_substr($val['tkr_name'], 0, 1, 'utf-8')?><span class="remark"><div class="txt_filter size_M"><i></i><i></i><i></i><i></i></div></span><span class="ticker"><span class="remark"><div class="txt_filter size_S"><i></i><i></i><i></i><i></i></div></span></span></a></td>
                                            <?php else :?>
                                            <a href="/<?=KW?>_stock/recommend_view/<?=$val['rc_id']?>"><?=$val['tkr_name']?><span class="ticker"><?=$val['rc_ticker']?></span></a></td>
                                            <?php endif;?>
                                            <td class="num">
                                ...
                                <table cellspacing="0" border="1" class="tableRanking sum_table">
                                    ...
                                    ?>
                                        <tr<?=($val['rc_profit_rate']<0) ? ' class="hide_line"':''?>>
                                            ...
                                            <a href="javascript:<?=($this->session->userdata('is_newlink')===TRUE) ? 'fnSinChungNew();':'fnSinChung();'?>" class="btn_free md-trigger"><?=$val['tkr_name']?><span class="ticker"><?=$val['rc_ticker']?></span></a></td>
                                            <?php else :?>
                                            <a href="/<?=KW?>_stock/recommend_view/<?=$val['rc_id']?>"><?=$val['tkr_name']?><span class="ticker"><?=$val['rc_ticker']?></span></a></td>
                                            <?php endif;?>
                ...
                function view_more() {
                    ...
                    page += 1;
                    $.post('/<?=KW?>_stock/ajax_get_recommend_list', {'page': page}, function(res) {
                        if($.trim(res).length) {
                ...
                $(function() {
                    $('.sub_top .chart_area').on('click', function() {
                        location.href = '/<?=KW?>_stock/recommend_view/' + $(this).data('id');
                    });
                });
                </script>
                <?php endif;?>
