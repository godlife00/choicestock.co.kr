...
class Common {
	...
	function alert($msg) {
		...
		if(strstr($_SERVER['HTTP_USER_AGENT'], 'HK_AOS')) {
			$TMP_HTML .="<script>HK.showToast('".$msg."');</script>";
		}
		else {
	...
	function get_wowtv_info($ticker) {
		$file_path = str_replace('hoon','datahero',WEBDATA).'/wowtv_data/'.$ticker;

		if( is_file($file_path) ) {
			$item = unserialize(file_get_contents(str_replace('hoon','datahero',WEBDATA).'/wowtv_data/'.$ticker));
			$item['last_price'] = sprintf('%.2f', $item['last_price']);
			$item['diff_rate'] = $item['diff_rate'];
			$item['diff_price'] =$item['diff_price'];
	...
	public function set_pricepoint($value, $type) {
		if($type=='1') {
			$val = explode('.', $value);
			if($val[0]>999) $val[0] = number_format($val[0]);
			$ret = $val[0].'.<b>'.$val[1].'</b>';
		}
		else {
	...
	/*
	public function reArrange($data=array()) {
		...
		foreach($chart_arr as $key=>$val) {
			if($year != '') {
				if($year == substr($key,0,4)) continue;
			}
			$year = substr($key,0,4);
			$chart_new[$key] = $val;
		}
	...
	*/
	...
	public function csAesEncryptSSL($data, $key) {
		$iv = str_repeat(chr(0), 16);
		$encdata = @openssl_encrypt($data, "AES-256-CBC", $key, false, $iv);
		return $encdata;
	}

	public function csAesDecryptSSL($data, $key) {
		$iv = str_repeat(chr(0), 16);
		$decdata = openssl_decrypt($data, "AES-256-CBC", $key, false, $iv);
		return $decdata;
	}
	...
	public function countingStar($score) {
		$data = '';	
		for($i = 1 ; $i <= 5 ; $i++) { 
			if($score >= $i) {
				$data .= '<span class="starR on">별1</span>';
			}
			else {
				if($i-$score <= 0.5) {
					$data .= '<span class="starR on half">별1</span>';
				}
				else {
					$data .= '<span class="starR">별1</span>';
				}
			}
		}
		return $data;
    ...
    //적정주가 비율
    public function view_fairrate($tkr_close, $mri_data) {
    /*
