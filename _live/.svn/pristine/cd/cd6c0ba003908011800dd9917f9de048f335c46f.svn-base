...
class Kw_main extends Kw_BaseMobile_Controller {
    ...
    public function index()
    {
        ...
        $data['meta_title'] = '투자를 쉽고 편리하게 - 미주미초이스';
		...
		if( is_file($file_path) ) {
			...
			foreach($morning_list as $morning) {
				if($morning['e_display_date'] <= date('Y-m-d H:i:s')) {
					...
					$cont_banner[$cont]['url'] = '/'.KW.'_stock/research_view/'.$morning['e_id'];
...
//echo '<pre>'; print_r($tab_stock_data);
    ...
    public function saveTicker() {
        ...
        $this->common->locationhref('/'.KW.'_main/search');
        exit;
    }

	public function service_info($type='') {
		if($type=='1') {
	        $this->load->view('/'.KW.'/main/service_info1');
		}
		else {
			if(date('YmdH') > '2021052916') {
		        $this->load->view('/'.KW.'/main/service_info1');
			}
			else {
		        $this->load->view('/'.KW.'/main/service_info');
			}
		}
    }
}
